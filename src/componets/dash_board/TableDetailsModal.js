  ...
  const centerSummaries = useMemo(() => {
    ...
    Object.entries(groupedByCenters).forEach(([kendraName, items]) => {
      const totalAllocated = items.reduce(
        (sum, item) =>
          sum +
          parseFloat(item.allocated_quantity || 0) * parseFloat(item.rate || 0),
        0
      );
      const totalUpdated = items.reduce(
        (sum, item) =>
          sum +
          parseFloat(item.updated_quantity || 0) * parseFloat(item.rate || 0),
        0
      );
...
th { background-color: #f2f2f2; font-size: 14px; font-weight: 600; }
...
`;
      ...
      // Just open for preview, no automatic printing
    } catch (error) {
  ...
  // Export kendra-specific data to PDF
  const exportKendraToPDF = (kendraName, kendraData) => {
    try {
      // Create a new window for preview
      const printWindow = window.open("", "_blank");

      // Generate HTML content for the kendra
      const htmlContent = `
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>${kendraName} - केंद्र विवरण रिपोर्ट</title>
<style>
body { font-family: Arial, sans-serif; margin: 20px; }
h1 { color: #2c3e50; text-align: center; }
h2 { color: #3498db; }
...
<div class="footer">
<p>रिपोर्ट तैयार की गई: ${new Date().toLocaleString("hi-IN")}</p>
<p>कुल ${kendraData.length} रिकॉर्ड का विश्लेषण</p>
</div>
</body>
</html>
`;
      ...
      console.error("Error exporting kendra to PDF:", error);
      alert("केंद्र पीडीएफ निर्यात में त्रुटि हुई। कृपया फिर से प्रयास करें।");
    }
  };
    ...
    try {
      ...
      const getSchemeDescription = (scheme) => {
        const descriptions = {
          ...
          "सरकारी कृषि योजना": "सरकार द्अनुसारा संचालित कृषि विकास कार्यक्रम",
          "राष्ट्रीय कृषि विकास योजना":
            "राष्ट्रीय स्तर पर कृषि उत्पादकता बढ़ाने के लिए",
          "प्रधानमंत्री फसल बीमा योजना": "फसल के जोखिम से बचाव के लिए",
          "किसान पेंशन योजना": "किसानों के वृद्धावस्था सुरक्षा के लिए",
          "मृदा स्वास्थ्य कार्ड योजना":
            "मिट्टी की गुणवत्ता जांच और सुधार के लिए",
        };
      ...
      const getGeographicalAnalysis = () => {
        ...
        uniqueSchemes.forEach((scheme) => {
          const schemeItems = tableData.filter(
            (item) => item.scheme_name === scheme
          );
          const vidhanSabhas = [
            ...new Set(schemeItems.map((item) => item.vidhan_sabha_name)),
          ].filter(Boolean);
          const vikasKhands = [
            ...new Set(schemeItems.map((item) => item.vikas_khand_name)),
          ].filter(Boolean);
          const centers = [
            ...new Set(schemeItems.map((item) => item.center_name)),
          ].filter(Boolean);
      ...
      const getComponentAnalysis = () => {
        ...
        uniqueComponents.forEach((component) => {
          const componentItems = tableData.filter(
            (item) => item.component === component
          );
          const schemes = [
            ...new Set(componentItems.map((item) => item.scheme_name)),
          ].filter(Boolean);
          const investments = [
            ...new Set(componentItems.map((item) => item.investment_name)),
          ].filter(Boolean);
          const sources = [
            ...new Set(componentItems.map((item) => item.source_of_receipt)),
          ].filter(Boolean);
          const vidhanSabhas = [
            ...new Set(componentItems.map((item) => item.vidhan_sabha_name)),
          ].filter(Boolean);
          const vikasKhands = [
            ...new Set(componentItems.map((item) => item.vikas_khand_name)),
          ].filter(Boolean);
          ...
          componentAnalysis.push({
            ...
            सप्लायर: sources.join(", "),
            "सप्लायरों की संख्या": sources.length,
            "शामिल विधानसभाएं": vidhanSabhas.join(", "),
            "विधानसभाओं की संख्या": vidhanSabhas.length,
            "शामिल विकासखंड": vikasKhands.join(", "),
            "विकासखंडों की संख्या": vikasKhands.length,
      ...
      const getRelationalMatrix = () => {
        ...
        uniqueSchemes.forEach((scheme) => {
          ...
          const schemeInvestments = [
            ...new Set(schemeItems.map((item) => item.investment_name)),
          ].filter(Boolean);
          const schemeSources = [
            ...new Set(schemeItems.map((item) => item.source_of_receipt)),
          ].filter(Boolean);
          const schemeVidhanSabhas = [
            ...new Set(schemeItems.map((item) => item.vidhan_sabha_name)),
          ].filter(Boolean);
          const schemeVikasKhands = [
            ...new Set(schemeItems.map((item) => item.vikas_khand_name)),
          ].filter(Boolean);
      ...
      const excelData = tableData.map((item) => ({
        ...
        "वितरण प्रतिशत": (
          (parseFloat(item.updated_quantity || 0) /
            parseFloat(item.allocated_quantity || 1)) *
          100
        ).toFixed(2),
        स्थिति:
          parseFloat(item.allocated_quantity || 0) -
      ...
      const executiveSummary = [
        ...
        [
          "केंद्रों की संख्या",
          new Set(tableData.map((item) => item.center_name)).size,
        ],
        ["", ""],
        ["योजना विवरण", ""],
        ...
        [
          "वितरण प्रतिशत",
          `${((totals.totalUpdated / totals.totalAllocated) * 100).toFixed(
            2
          )}%`,
        ],
        [
          "बची हुई राशि प्रतिशत",
          `${((totals.totalRemaining / totals.totalAllocated) * 100).toFixed(
            2
          )}%`,
        ],
      ];
      ...
      // 3. Kendra-wise Detailed Data Sheets
      Object.entries(groupedByCenters).forEach(([kendraName, kendraData]) => {
        // Create table data exactly like PDF format
        const kendraTableData = [
          [
            "क्र.सं.",
            "विधानसभा",
            "विकासखंड",
        ...
        kendraData.forEach((item, index) => {
          ...
          kendraTableData.push([
            ...
            `₹${allocated.toFixed(2)}`,
            parseFloat(item.updated_quantity || 0).toFixed(2),
            `₹${sold.toFixed(2)}`,
            `₹${remaining.toFixed(2)}`,
          ]);
        });

        const kendraSheet = XLSX.utils.aoa_to_sheet(kendraTableData);
        XLSX.utils.book_append_sheet(
          wb,
          kendraSheet,
          `${kendraName.substring(0, 20)} डेटा`
        );
      });

      // 4. Comprehensive Summary data
      const summaryData = [
        ["संपूर्ण सारांश", ""],
        ["रिकॉर्ड विवरण", "मान"],
        ["कुल रिकॉर्ड", tableData.length],
        ...
        [
          "प्रभावशीलता",
          totals.totalAllocated > 0 ? "उत्कृष्ट" : "मूल्यांकन आवश्यक",
        ],
      ];
      ...
      const locationDetails = uniqueVikasKhands.map((location) => {
        const locationItems = tableData.filter(
          (item) => item.vikas_khand_name === location
        );
        const allocated = locationItems.reduce(
          (sum, item) =>
            sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
          0
        );
        ...
        const centers = [
          ...new Set(locationItems.map((item) => item.center_name)),
        ].filter(Boolean);
        const schemes = [
          ...new Set(locationItems.map((item) => item.scheme_name)),
        ].filter(Boolean);
        const totalAllocatedQuantity = locationItems.reduce(
          (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
          0
        );
      ...
      const sourceDetails = uniqueSources.map((source) => {
        const sourceItems = tableData.filter(
          (item) => item.source_of_receipt === source
        );
        const allocated = sourceItems.reduce(
          (sum, item) =>
            sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
          0
        );
        const sold = sourceItems.reduce(
          (sum, item) =>
            sum + parseFloat(item.updated_quantity) * parseFloat(item.rate),
          0
        );
      ...
      XLSX.utils.book_append_sheet(wb, sourceSheet, "सप्लायर विवरण");

      // 8. Investment-wise breakdown
      const investmentDetails = uniqueInvestments.map((investment) => {
        const investmentItems = tableData.filter(
      ...
      // 8.5. Sub-Investment-wise breakdown
      const uniqueSubInvestments = [
        ...new Set(tableData.map((item) => item.sub_investment_name)),
      ].filter(Boolean);

      const subInvestmentDetails = uniqueSubInvestments.map((subInvestment) => {
        const subInvestmentItems = tableData.filter(
          (item) => item.sub_investment_name === subInvestment
        );
        const allocated = subInvestmentItems.reduce(
          (sum, item) =>
            sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
          0
        );
      ...
      const subInvestmentSheet = XLSX.utils.json_to_sheet(subInvestmentDetails);
      XLSX.utils.book_append_sheet(wb, subInvestmentSheet, "उप-निवेश विवरण");

      // 9. Kendra-wise Summary Sheet
      if (Object.keys(centerSummaries).length > 0) {
        const kendraSummaryData = Object.entries(centerSummaries).map(
          ([kendraName, summary]) => ({
            ...
            "वितरण प्रतिशत": `${summary.distributionPercentage}%`,
          })
        );
        const kendraSheet = XLSX.utils.json_to_sheet(kendraSummaryData);
        XLSX.utils.book_append_sheet(wb, kendraSheet, "केंद्र सारांश");
      }

      // 10. Kendra Comparison Summary Sheet
      if (Object.keys(centerSummaries).length > 1) {
        ...
        Object.entries(centerSummaries).forEach(([kendraName, summary]) => {
          comparisonData.push([
            kendraName,
            summary.recordCount,
        ...
        XLSX.utils.book_append_sheet(wb, comparisonSheet, "तुलनात्मक सारांश");
      }

      // 11. Scheme-wise Comparison Sheet
      if (Object.keys(centerSummaries).length > 1) {
        ...
        uniqueSchemes.forEach((scheme) => {
          ...
          Object.entries(groupedByCenters).forEach(
            ([kendraName, kendraData]) => {
              ...
              if (schemeItems.length > 0) {
                ...
                const percentage =
                  allocated > 0
                    ? ((sold / allocated) * 100).toFixed(2)
                    : "0.00";
                const allocatedQuantity = schemeItems.reduce(
                  (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
                  0
                );
          ...
          schemeComparison.forEach((data) => {
            schemeComparisonData.push([
              data.scheme,
              data.kendraName,
              data.recordCount,
              data.allocatedQuantity.toFixed(2),
              formatCurrency(data.allocated),
              formatCurrency(data.sold),
        ...
        const schemeComparisonSheet =
          XLSX.utils.aoa_to_sheet(schemeComparisonData);
        XLSX.utils.book_append_sheet(wb, schemeComparisonSheet, "योजना तुलना");
      }

      // 12. Component-wise Comparison Sheet
      if (Object.keys(centerSummaries).length > 1) {
        ...
        uniqueComponents.forEach((component) => {
              ...
              if (componentItems.length > 0) {
                ...
                const percentage =
                  allocated > 0
                    ? ((sold / allocated) * 100).toFixed(2)
                    : "0.00";
                const allocatedQuantity = componentItems.reduce(
                  (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
                  0
                );
          ...
          componentComparison.forEach((data) => {
        ...
        const componentComparisonSheet = XLSX.utils.aoa_to_sheet(
          componentComparisonData
        );
        XLSX.utils.book_append_sheet(wb, componentComparisonSheet, "घटक तुलना");
      }

      // 13.5. Sub-Investment-wise Comparison Sheet
      if (Object.keys(centerSummaries).length > 1) {
        const subInvestmentComparisonData = [
          [
            "उप-निवेश",
            "केंद्र",
            "रिकॉर्ड",
        ...
        uniqueSubInvestments.forEach((subInvestment) => {
          const subInvestmentComparison = [];

          Object.entries(groupedByCenters).forEach(
            ([kendraName, kendraData]) => {
              const subInvestmentItems = kendraData.filter(
                (item) => item.sub_investment_name === subInvestment
              );
              if (subInvestmentItems.length > 0) {
                const allocated = subInvestmentItems.reduce(
                  (sum, item) =>
                    sum +
                    parseFloat(item.allocated_quantity) * parseFloat(item.rate),
                ...
                const percentage =
                  allocated > 0
                    ? ((sold / allocated) * 100).toFixed(2)
                    : "0.00";
                const allocatedQuantity = subInvestmentItems.reduce(
                  (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
                  0
                );

                subInvestmentComparison.push({
                  subInvestment,
                  kendraName,
                  recordCount: subInvestmentItems.length,
                  allocatedQuantity,
                  allocated,
                  sold,
                  remaining,
          ...
          subInvestmentComparison.forEach((data) => {
            subInvestmentComparisonData.push([
              data.subInvestment,
              data.kendraName,
              data.recordCount,
              data.allocatedQuantity.toFixed(2),
        ...
        const subInvestmentComparisonSheet = XLSX.utils.aoa_to_sheet(
          subInvestmentComparisonData
        );
        XLSX.utils.book_append_sheet(
          wb,
          subInvestmentComparisonSheet,
          "उप-निवेश तुलना"
        );
      }

      // 14. Investment-wise Comparison Sheet
      if (Object.keys(centerSummaries).length > 1) {
        ...
        uniqueInvestments.forEach((investment) => {
              ...
              if (investmentItems.length > 0) {
                ...
                const percentage =
                  allocated > 0
                    ? ((sold / allocated) * 100).toFixed(2)
                    : "0.00";
                const allocatedQuantity = investmentItems.reduce(
                  (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
                  0
                );

                investmentComparison.push({
                  investment,
                  kendraName,
          ...
          investmentComparison.forEach((data) => {
        ...
        const investmentComparisonSheet = XLSX.utils.aoa_to_sheet(
          investmentComparisonData
        );
        XLSX.utils.book_append_sheet(
          wb,
          investmentComparisonSheet,
          "निवेश तुलना"
        );
      }

      // 14. Source-wise Comparison Sheet
      if (Object.keys(centerSummaries).length > 1) {
              ...
              if (sourceItems.length > 0) {
                ...
                const percentage =
                  allocated > 0
                    ? ((sold / allocated) * 100).toFixed(2)
                    : "0.00";
                const allocatedQuantity = sourceItems.reduce(
                  (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
                  0
                );
        ...
        const sourceComparisonSheet =
          XLSX.utils.aoa_to_sheet(sourceComparisonData);
        XLSX.utils.book_append_sheet(
          wb,
      ...
      // Export file
      const fileName = `${centerName || "data"}_संपूर्ण_रिपोर्ट_${
        new Date().toISOString().split("T")[0]
      }.xlsx`;
      XLSX.writeFile(wb, fileName);
...
<div class="summary-label">सप्लायर</div>
</div>
</div>
</div>
...
<!-- Kendra-wise Summary -->
<div class="section">
<div class="section-title">केंद्र अनुसार सारांश</div>
${Object.entries(centerSummaries)
  .map(
    ([kendraName, summary]) => `
...
${Object.entries(groupedByCenters)
  .map(([kendraName, kendraData]) => {
    const kendraSchemes = [
      ...new Set(kendraData.map((item) => item.scheme_name)),
    ].filter(Boolean);
    ...
    const kendraVidhanSabhas = [
      ...new Set(kendraData.map((item) => item.vidhan_sabha_name)),
    ].filter(Boolean);
    const kendraVikasKhands = [
      ...new Set(kendraData.map((item) => item.vikas_khand_name)),
    ].filter(Boolean);
...
${kendraSchemes
  .map((scheme) => {
    ...
    const geoDistribution = Object.entries(geoMapping)
      .map(
        ([vidhanSabha, vikasKhandSet]) =>
    ...
    const componentMapping = schemeItems.reduce((acc, item) => {
      ...
      if (location && location !== " → ")
        acc[component].locations.add(location);

      return acc;
    }, {});

    const componentDetails = Object.entries(componentMapping)
      .map(
        ([component, data]) =>
          `${component} → निवेश: ${
...
<span class="value">${
      components.length > 0 ? components.join(", ") : "कोई घटक नहीं"
    } (${components.length} घटक)</span>
</div>
<div class="detail-row">
<span class="label">संबंधित निवेश:</span>
<span class="value">${
      investments.length > 0 ? investments.join(", ") : "कोई निवेश नहीं"
    } (${investments.length} निवेश)</span>
</div>
<div class="detail-row">
<span class="label">सप्लायर:</span>
<span class="value">${
      sources.length > 0 ? sources.join(", ") : "कोई सप्लायर नहीं"
    } (${sources.length} सप्लायर)</span>
</div>
<div class="detail-row">
<span class="label">उपस्थित विधानसभाएं:</span>
<span class="value">${
      vidhanSabhas.length > 0 ? vidhanSabhas.join(", ") : "कोई विधानसभा नहीं"
    } (${vidhanSabhas.length} विधानसभाएं)</span>
</div>
<div class="detail-row">
<span class="label">शामिल विकासखंड:</span>
<span class="value">${
      vikasKhands.length > 0 ? vikasKhands.join(", ") : "कोई विकासखंड नहीं"
    } (${vikasKhands.length} विकासखंड)</span>
</div>
<div class="detail-row">
...
<td class="highlight"><strong>${formatCurrency(
      kendraData.reduce(
        (sum, item) =>
          sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
        0
...
${kendraComponents
  .map((component) => {
    ...
    const relationshipDetails = Object.entries(schemeMapping)
      .map(
        ([scheme, data]) =>
          `${scheme} → निवेश: ${
            Array.from(data.investments).join(", ") || "कोई नहीं"
...
<span class="value">${
      schemes.length > 0 ? schemes.join(", ") : "कोई योजना नहीं"
    }</span>
</div>
<div class="detail-row">
<span class="label">योजना-अनुसार विवरण:</span>
...
<div class="detail-row">
<span class="label">संबंधित सप्लायर:</span>
<span class="value">${
      sources.length > 0 ? sources.join(", ") : "कोई सप्लायर नहीं"
    }</span>
</div>
...
${kendraVikasKhands
  .map((location) => {
    const locationItems = kendraData.filter(
      (item) => item.vikas_khand_name === location
    );
    const allocated = locationItems.reduce(
      (sum, item) =>
...
${Object.entries(groupedByCenters)
  .map(([kendraName, kendraData]) => {
    ...
    return `
<div class="section">
<div class="section-title">${kendraName} - निवेश अनुसार संबंधात्मक विस्तृत सारांश</div>
...
${kendraInvestments
  .map((investment) => {
    ...
    const schemes = [
      ...new Set(investmentItems.map((item) => item.scheme_name)),
    ].filter(Boolean);
    const components = [
      ...new Set(investmentItems.map((item) => item.component)),
    ].filter(Boolean);
    const sources = [
      ...new Set(investmentItems.map((item) => item.source_of_receipt)),
    ].filter(Boolean);
    const vidhanSabhas = [
      ...new Set(investmentItems.map((item) => item.vidhan_sabha_name)),
    ].filter(Boolean);
    const vikasKhands = [
      ...new Set(investmentItems.map((item) => item.vikas_khand_name)),
    ].filter(Boolean);
    const allocated = investmentItems.reduce(
      (sum, item) =>
        sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
...
<span class="value">${
      vidhanSabhas.length > 0 ? vidhanSabhas.join(", ") : "कोई विधानसभा नहीं"
    } (${vidhanSabhaCount} विधानसभाएं)</span>
</div>
<div class="detail-row">
<span class="label">शामिल विकासखंड:</span>
<span class="value">${
      vikasKhands.length > 0 ? vikasKhands.join(", ") : "कोई विकासखंड नहीं"
    } (${vikasKhandCount} विकासखंड)</span>
</div>
<div class="detail-row">
<span class="label">कुल स्थानों की संख्या:</span>
...
<th>${kendraInvestments.reduce((sum, investment) => {
      const items = kendraData.filter(
        (item) => item.investment_name === investment
      );
      const vidhanSabhas = [
        ...new Set(items.map((item) => item.vidhan_sabha_name)),
      ].filter(Boolean);
      const vikasKhands = [
        ...new Set(items.map((item) => item.vikas_khand_name)),
      ].filter(Boolean);
...
<th>${
      [...new Set(kendraData.map((item) => item.component))].filter(Boolean)
        .length
    }</th>
<th>${
      [...new Set(kendraData.map((item) => item.source_of_receipt))].filter(
        Boolean
      ).length
    }</th>
<th>${
      [...new Set(kendraData.map((item) => item.vidhan_sabha_name))].filter(
        Boolean
      ).length
    }</th>
<th>${(
...
<!-- Kendra-wise Sub-Investment Breakdowns -->
${Object.entries(groupedByCenters)
  .map(([kendraName, kendraData]) => {
    const kendraSubInvestments = [
      ...new Set(kendraData.map((item) => item.sub_investment_name)),
    ].filter(Boolean);

    if (kendraSubInvestments.length === 0) return "";

    return `
<div class="section">
<div class="section-title">${kendraName} - उप-निवेश अनुसार संबंधात्मक विस्तृत सारांश</div>
<div class="sub-investment-comprehensive">
<h4>${kendraName} में उप-निवेशों का संबंधात्मक विवरण और उनकी उपस्थिति:</h4>
<ul>
${kendraSubInvestments
  .map((subInvestment) => {
    const subInvestmentItems = kendraData.filter(
      (item) => item.sub_investment_name === subInvestment
    );
    const schemes = [
      ...new Set(subInvestmentItems.map((item) => item.scheme_name)),
    ].filter(Boolean);
    const components = [
      ...new Set(subInvestmentItems.map((item) => item.component)),
    ].filter(Boolean);
    const investments = [
      ...new Set(subInvestmentItems.map((item) => item.investment_name)),
    ].filter(Boolean);
    const sources = [
      ...new Set(subInvestmentItems.map((item) => item.source_of_receipt)),
    ].filter(Boolean);
    const vidhanSabhas = [
      ...new Set(subInvestmentItems.map((item) => item.vidhan_sabha_name)),
    ].filter(Boolean);
    const vikasKhands = [
      ...new Set(subInvestmentItems.map((item) => item.vikas_khand_name)),
    ].filter(Boolean);
    const allocated = subInvestmentItems.reduce(
      (sum, item) =>
        sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
    ...
    return `
<li><strong>${subInvestment}</strong>
<div class="sub-investment-details">
<div class="detail-row">
<span class="label">संबंधित निवेश:</span>
<span class="value">${
      investments.length > 0 ? investments.join(", ") : "कोई निवेश नहीं"
...
<table class="data-table sub-investment-summary-table">
<thead>
<tr>
...
<th>${kendraName} कुल</th>
<th>${kendraData.length}</th>
<th>${kendraSubInvestments.reduce((sum, subInvestment) => {
      const items = kendraData.filter(
        (item) => item.sub_investment_name === subInvestment
      );
      const vidhanSabhas = [
...
<h4>${kendraName} में सप्लायरों का विवरण और वितरण:</h4>
<ul>
${kendraSources
  .map((source) => {
    const sourceItems = kendraData.filter(
    ...
    const schemes = [
      ...new Set(sourceItems.map((item) => item.scheme_name)),
    ].filter(Boolean);
    const components = [
      ...new Set(sourceItems.map((item) => item.component)),
    ].filter(Boolean);
    const investments = [
      ...new Set(sourceItems.map((item) => item.investment_name)),
    ].filter(Boolean);
    const vidhanSabhas = [
      ...new Set(sourceItems.map((item) => item.vidhan_sabha_name)),
    ].filter(Boolean);
    const vikasKhands = [
      ...new Set(sourceItems.map((item) => item.vikas_khand_name)),
    ].filter(Boolean);
    const allocated = sourceItems.reduce(
      (sum, item) =>
        sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
...
<div class="footer">
<p><strong>रिपोर्ट तैयार की गई:</strong> ${new Date().toLocaleString(
        "hi-IN"
      )}</p>
...
`;
  ...
  const getTooltipData = (itemType, itemValue, filteredData) => {
    const relevantData = filteredData.filter((item) => {
      switch (itemType) {
        case "scheme":
          return item.scheme_name === itemValue;
        case "investment":
          return item.investment_name === itemValue;
        case "component":
          return item.component === itemValue;
        case "source":
          return item.source_of_receipt === itemValue;
        case "vidhanSabha":
          return item.vidhan_sabha_name === itemValue;
        case "vikasKhand":
          return item.vikas_khand_name === itemValue;
        default:
          return false;
      }
    });
    ...
    const totalAllocated = relevantData.reduce(
      (sum, item) =>
        sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
      0
    );
    const totalUpdated = relevantData.reduce(
      (sum, item) =>
        sum + parseFloat(item.updated_quantity) * parseFloat(item.rate),
      0
    );
    ...
    const uniqueVidhanSabhas = [
      ...new Set(relevantData.map((item) => item.vidhan_sabha_name)),
    ].filter(Boolean);
    const uniqueVikasKhands = [
      ...new Set(relevantData.map((item) => item.vikas_khand_name)),
    ].filter(Boolean);
    ...
    return {
      ...
      uniqueVidhanSabhas,
      uniqueVikasKhands,
      allocatedQuantity: relevantData.reduce(
        (sum, item) => sum + parseFloat(item.allocated_quantity || 0),
        0
      ),
      updatedQuantity: relevantData.reduce(
        (sum, item) => sum + parseFloat(item.updated_quantity || 0),
        0
      ),
  ...
  const getTooltipContent = (itemType, itemValue, tooltipData, allData) => {
    ...
    const locations =
      tooltipData.uniqueVidhanSabhas.length > 0
        ? tooltipData.uniqueVidhanSabhas.join(", ")
        : "N/A";

    const vikasKhands =
      tooltipData.uniqueVikasKhands.length > 0
    ...
    if (itemType === "scheme") {
      const relatedComponents = [
        ...new Set(
          allData
            .filter((item) => item.scheme_name === itemValue)
      ...
      relatedInfo = `घटक: ${relatedComponents.join(
        ", "
      )}\nसप्लायर: ${relatedSources.join(", ")}`;
    } else if (itemType === "component") {
      const relatedSchemes = [
        ...new Set(
          allData
            .filter((item) => item.component === itemValue)
      ...
      const relatedSchemes = [
        ...new Set(
          allData
            .filter((item) => item.source_of_receipt === itemValue)
            .map((item) => item.scheme_name)
        ),
      ].filter(Boolean);
      const relatedComponents = [
      ...
      const relatedSchemes = [
        ...new Set(
          allData
            .filter((item) => item.investment_name === itemValue)
            .map((item) => item.scheme_name)
        ),
      ].filter(Boolean);
      const relatedComponents = [
...
विकासखंड: ${vikasKhands}`;
  ...
  const uniqueVidhanSabhas = useMemo(() => {
    return [...new Set(tableData.map((item) => item.vidhan_sabha_name))]
      .filter(Boolean)
      .sort();
  }, [tableData]);

  const uniqueVikasKhands = useMemo(() => {
    return [...new Set(tableData.map((item) => item.vikas_khand_name))]
      .filter(Boolean)
      .sort();
  }, [tableData]);

  const uniqueSchemes = useMemo(() => {
    return [...new Set(tableData.map((item) => item.scheme_name))]
      .filter(Boolean)
      .sort();
  }, [tableData]);

  const uniqueInvestments = useMemo(() => {
    return [...new Set(tableData.map((item) => item.investment_name))]
      .filter(Boolean)
      .sort();
  }, [tableData]);
  ...
  const uniqueSources = useMemo(() => {
    return [...new Set(tableData.map((item) => item.source_of_receipt))]
      .filter(Boolean)
      .sort();
  }, [tableData]);
  ...
  const totals = useMemo(() => {
    ...
    const totalUpdated = tableData.reduce(
      (sum, item) =>
        sum + parseFloat(item.updated_quantity) * parseFloat(item.rate),
      0
    );
  ...
  const showAllocationDetails = (item, itemType) => {
    setAllocationDetails((prev) => {
      const exists = prev.find(
        (d) => d.selectedItem === item && d.itemType === itemType
      );
      if (exists) {
        return prev.filter(
          (d) => !(d.selectedItem === item && d.itemType === itemType)
  ...
  const closeAllocationDetails = (item, itemType) => {
    setAllocationDetails((prev) =>
      prev.filter((d) => !(d.selectedItem === item && d.itemType === itemType))
    );
  };
  ...
  const renderSectionBreakdown = (details, sectionType, closeFunction) => {
    ...
    const filterKey =
      details.itemType === "scheme" ? "scheme_name" : "component";
    ...
    const getTitle = () => {
      ...
      switch (sectionType) {
        case "allocation":
          return `${typeLabel} आवंटन विवरण: ${itemName}`;
        case "sales":
          return `${typeLabel}वितरण: ${itemName}`;
        case "remaining":
          return `${typeLabel} शेष राशि विवरण: ${itemName}`;
        default:
          return `${itemName} विवरण`;
      }
    };
        ...
        <Table striped bordered hover size="sm">
          ...
          <tbody>
            {itemData.map((record, index) => {
              const allocated =
                parseFloat(record.allocated_quantity || 0) *
  ...
  const filteredData = useMemo(
    () => getFilteredData(),
    [tableData, selectedSchemes, selectedComponents]
  );

  const filteredUniqueSchemes = useMemo(() => {
    return [...new Set(filteredData.map((item) => item.scheme_name))]
      .filter(Boolean)
      .sort();
  }, [filteredData]);

  const filteredUniqueInvestments = useMemo(() => {
    return [...new Set(filteredData.map((item) => item.investment_name))]
      .filter(Boolean)
      .sort();
  }, [filteredData]);

  const filteredUniqueComponents = useMemo(() => {
    return [...new Set(filteredData.map((item) => item.component))]
      .filter(Boolean)
      .sort();
  }, [filteredData]);

  const filteredUniqueSources = useMemo(() => {
    return [...new Set(filteredData.map((item) => item.source_of_receipt))]
      .filter(Boolean)
      .sort();
  }, [filteredData]);
  ...
  const sourceFilteredUniqueSchemes = useMemo(() => {
    return [...new Set(sourceFilteredData.map((item) => item.scheme_name))]
      .filter(Boolean)
      .sort();
  }, [sourceFilteredData]);

  const sourceFilteredUniqueInvestments = useMemo(() => {
    return [...new Set(sourceFilteredData.map((item) => item.investment_name))]
      .filter(Boolean)
      .sort();
  }, [sourceFilteredData]);
  ...
  const sourceFilteredUniqueSources = useMemo(() => {
    return [
      ...new Set(sourceFilteredData.map((item) => item.source_of_receipt)),
    ]
      .filter(Boolean)
      .sort();
  }, [sourceFilteredData]);
  ...
  const sourceFilteredTotals = useMemo(() => {
    const totalAllocated = sourceFilteredData.reduce(
      (sum, item) =>
        sum + parseFloat(item.allocated_quantity) * parseFloat(item.rate),
      0
    );
    const totalUpdated = sourceFilteredData.reduce(
      (sum, item) =>
        sum + parseFloat(item.updated_quantity) * parseFloat(item.rate),
      0
    );
  ...
  const filteredTotals = useMemo(() => {
    ...
    const totalUpdated = filteredData.reduce(
      (sum, item) =>
        sum + parseFloat(item.updated_quantity) * parseFloat(item.rate),
      0
    );
              ...
              <Button
                ...
                onClick={() => {
                  if (pdfWindow && !pdfWindow.closed) {
                    ...
                    alert(
                      "PDF विंडो बंद है। कृपया पहले PDF पूर्वावलोकन खोलें।"
                    );
                    setShowPrintButton(false);
                  }
                }}
      ...
      <Modal.Body>
        ...
        <Card className="mb-3">
          <Card.Header className="fillter-heading">
            <h6 className="mb-0">
              <FaChartBar className="me-2" /> सारांश आँकड़े
            </h6>
          </Card.Header>
          <Card.Body>
            <Row>
            ...
            <Row>
              ...
              <Col md={6} className="text-end">
                <div className="d-flex flex-row align-items-end">
                  ...
                  <span
                    className="badge bg-light text-dark mb-1 clickable-badge"
                    title={`कुल आवंटित राशि: ${formatCurrency(
                      totals.totalAllocated
                    )} - सभी मदों के लिए आवंटित धनराशि | क्लिक करें विवरण देखने के लिए`}
                    data-bs-toggle="tooltip"
                    data-bs-placement="left"
                  ...
                  <span
                    className="badge bg-light text-dark mb-1 clickable-badge"
                    title={`कुल वितरण राशि: ${formatCurrency(
                      totals.totalUpdated
                    )} - अब तक वितरण धनराशि | क्लिक करें विवरण देखने के लिए`}
                    data-bs-toggle="tooltip"
                    data-bs-placement="left"
                  ...
                  <span
                    ...
                    title={`शेष राशि: ${formatCurrency(
                      totals.totalRemaining
                    )} - अभी भी बेचने के लिए शेष धनराशि | क्लिक करें विवरण देखने के लिए`}
                    data-bs-toggle="tooltip"
                    data-bs-placement="left"
        ...
        <Card className="mb-3" id="places-section">
          ...
          >
            <span>
              <FaBuilding className="me-2" /> विधानसभा और विकासखंड
            </span>
            ...
            <Card.Body>
              <div className="text-center mb-2">
                <h6 className="text-primary fw-bold">
                  विकासखंड: {uniqueVikasKhands.length}
                </h6>
              </div>
              <div className="places-grid">
                {uniqueVikasKhands.map((vikasKhand, index) => (
                  <Badge
                    key={index}
                    bg={
                      selectedVikasKhand === vikasKhand ? "success" : "primary"
                    }
                    ...
                    title={
                      selectedVikasKhand === vikasKhand
                        ...
                        : "क्लिक करें तालिका देखने के लिए"
                    }
                  >
                    {vikasKhand}
                    {selectedVikasKhand === vikasKhand && (
                ...
                <div className="mt-3">
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    <h6 className="text-success fw-bold mb-0">
                      {selectedVikasKhand} - विवरण
                    </h6>
                    <Button
                      variant="outline-secondary"
                      size="sm"
                      onClick={() => setSelectedVikasKhand(null)}
                    >
                      <FaTimes />
                    </Button>
                  </div>
                  ...
                  >
                    ...
                    <tbody>
                      {tableData
                        .filter(
                          (item) => item.vikas_khand_name === selectedVikasKhand
                        )
                        .map((item, index) => {
                          const allocated =
        ...
        <Card className="mb-3" id="allocation-section">
          ...
          >
            <span>
              <FaPiggyBank className="me-2" /> आवंटन विवरण
            </span>
            <div className="d-flex align-items-center gap-2">
                ...
                onClick={(e) => {
                  ...
                  allocationDetails.forEach((detail) => {
                    if (detail.selectedItem) {
                      ...
                      const filterKey =
                        detail.itemType === "scheme"
                          ? "scheme_name"
                          : "component";
                      const itemData = tableData.filter(
                        (item) => item[filterKey] === itemName
                      );
                      ...
                      itemData.forEach((record) => {
                        breakdownData.push({
                          ...
                          दर: record.rate,
                          "आवंटित राशि": formatCurrency(
                            parseFloat(record.allocated_quantity) *
          ...
          <Collapse in={!collapsedSections.allocation}>
            <Card.Body>
              <Row>
                <Col md={6}>
                  ...
                  {uniqueSchemes.map((scheme, index) => {
                    const schemeData = tableData.filter(
                      (item) => item.scheme_name === scheme
                    );
                    const totalAllocated = schemeData.reduce(
                      (sum, item) =>
                        sum +
                        parseFloat(item.allocated_quantity) *
                          parseFloat(item.rate),
                    ...
                    const isSelected = allocationDetails.some(
                      (d) =>
                        d.selectedItem === scheme && d.itemType === "scheme"
                    );
                    return (
                      <div
                        key={index}
                ...
                <Col md={6}>
                  ...
                  {uniqueComponents.map((component, index) => {
                    const componentData = tableData.filter(
                      (item) => item.component === component
                    );
                    const totalAllocated = componentData.reduce(
                      (sum, item) =>
                        sum +
                        parseFloat(item.allocated_quantity) *
                          parseFloat(item.rate),
                    ...
                    const isSelected = allocationDetails.some(
                      (d) =>
                        d.selectedItem === component &&
                        d.itemType === "component"
                    );
                    return (
                      <div
                        key={index}
                        ...
                        onClick={() =>
                          showAllocationDetails(component, "component")
                        }
                        style={{ cursor: "pointer" }}
                        title="क्लिक करें विस्तृत आवंटन विवरण देखने के लिए"
                      >
              ...
              {allocationDetails.map((detail) =>
                renderSectionBreakdown(detail, "allocation", () =>
                  closeAllocationDetails(detail.selectedItem, detail.itemType)
                )
              )}
            </Card.Body>
          </Collapse>
                    ...
                    if (detail.selectedItem) {
                      ...
                      itemData.forEach((record) => {
                        breakdownData.push({
                          ...
                          दर: record.rate,
                          "वितरण राशि": formatCurrency(
                            parseFloat(record.updated_quantity) *
                              parseFloat(record.rate)
                          ),
                        });
                      });
          ...
          <Collapse in={!collapsedSections.sales}>
            <Card.Body>
              ...
              <Row>
                <Col md={6}>
                  ...
                  {uniqueSchemes.map((scheme, index) => {
                    ...
                    const totalSold = schemeData.reduce(
                      (sum, item) =>
                        sum +
                    ...
                    const isSelected = salesDetails.some(
                      (d) =>
                        d.selectedItem === scheme && d.itemType === "scheme"
                    );
                    return (
                ...
                <Col md={6}>
                  ...
                  {uniqueComponents.map((component, index) => {
                    ...
                    const totalSold = componentData.reduce(
                      (sum, item) =>
                        sum +
                        parseFloat(item.updated_quantity) *
                          parseFloat(item.rate),
              ...
              {salesDetails.map((detail) =>
                renderSectionBreakdown(detail, "sales", () =>
                  closeSalesDetails(detail.selectedItem, detail.itemType)
                )
              )}
            </Card.Body>
          </Collapse>
        ...
        <Card className="mb-3" id="remaining-section">
          ...
          >
            <span>
              <FaLayerGroup className="me-2" /> शेष राशि विवरण
            </span>
            <div className="d-flex align-items-center gap-2">
              {collapsedSections.remaining ? (
                <FaChevronDown />
              ) : (
                <FaChevronUp />
              )}
                  ...
                  {uniqueComponents.map((component, index) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          showRemainingDetails(component, "component")
                        }
                        style={{ cursor: "pointer" }}
                        title="क्लिक करें विस्तृत शेष राशि विवरण देखने के लिए"
                      >
        ...
        <Card className="mb-3" id="hierarchy-section">
          ...
          >
            <span>
              <FaList className="me-2" /> पदानुक्रमिक संरचना
            </span>
            <div className="d-flex align-items-center gap-2">
              {collapsedSections.hierarchy ? (
                <FaChevronDown />
              ) : (
                <FaChevronUp />
              )}
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  const hierarchyData = [];
                  Object.entries(hierarchicalData).forEach(
                    ([vidhanSabha, data]) => {
                        ...
                        <div key={vidhanSabha} className="mb-1">
                          <div className="d-flex justify-content-between align-items-center">
                            ...
                            <span className="badge bg-light text-dark">
                              विकासखंड {data.vikasKhands.length}
                            </span>
                          </div>
                        ...
                        <div key={vidhanSabha} className="mb-1">
                          ...
                          <div className="compact-list">
                            {data.schemes.map((scheme, index) => {
                              ...
                              const isCollapsed =
                                collapsedSchemes.has(schemeKey);
                              const schemeInvestments =
                                data.schemeInvestments[scheme] || [];
                              ...
                              return (
                                <div key={index} className="compact-item">
                                  <div
                                    ...
                                    onClick={() =>
                                      toggleSchemeCollapse(schemeKey)
                                    }
                                    style={{ cursor: "pointer" }}
                                  >
        ...
        <Card className="mb-3" id="filter-section">
          ...
          >
            <span>
              <FaPiggyBank className="me-2" /> योजनाएं और निवेश फ़िल्टर
            </span>
            <div className="d-flex align-items-center gap-2">
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  XLSX.utils.book_append_sheet(wb, ws, "फ़िल्टर डेटा");
                  XLSX.writeFile(
                    wb,
                    `फ़िल्टर_डेटा_${
                      new Date().toISOString().split("T")[0]
                ...
                title="फ़िल्टर डेटा Excel में निर्यात"
              >
                <FaFileExcel />
              </Button>
              <Button
                ...
                onClick={(e) => {
                  ...
                  const filterData = filteredData.map((item) => ({
                    ...
                    वितरण: formatCurrency(
                      parseFloat(item.updated_quantity) * parseFloat(item.rate)
                    ),
                  }));
                  exportSectionToPDF("फ़िल्टर_डेटा", filterData);
                }}
                title="फ़िल्टर डेटा PDF में निर्यात"
              >
                <FaFilePdf />
              </Button>
            </div>
                  ...
                  <div className="selection-panel">
                    ...
                    <div className="mb-3">
                      <h6 className="fw-bold mb-2 text-info">
                        सभी योजनाएं ({uniqueSchemes.length})
                      </h6>
                        ...
                        {uniqueSchemes.map((scheme, index) => {
                          ...
                          return (
                            <Badge
                              key={index}
                              bg={
                                selectedSchemes.has(scheme) ? "info" : "light"
                              }
                              text={
                                selectedSchemes.has(scheme) ? "dark" : "dark"
                              }
                              className="p-2 selectable-badge"
                              style={{ cursor: "pointer" }}
                              onClick={() => toggleScheme(scheme)}
                              title={
                                selectedSchemes.has(scheme) ? "हटाएं" : "चुनें"
                              }
                              data-bs-toggle="tooltip"
                              data-bs-placement="top"
                            >
                              {scheme}
                              {selectedSchemes.has(scheme) && (
                                <span className="ms-1">✓</span>
                              )}
                            </Badge>
                          );
                  ...
                  <div className="results-panel">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      ...
                      <div className="text-end">
                        <small className="text-muted">
                          चयनित:{" "}
                          {selectedSchemes.size + selectedComponents.size}
                        </small>
                      </div>
                    </div>
                        ...
                        <div className="row text-center">
                          ...
                          <div className="col-4">
                            <div className="fw-bold text-success">
                              {formatCurrency(filteredTotals.totalAllocated)}
                            </div>
                            <small className="text-muted">आवंटित</small>
                          </div>
                          <div className="col-4">
                            <div className="fw-bold text-warning">
                              {formatCurrency(filteredTotals.totalUpdated)}
                            </div>
                        ...
                        <div className="category-section">
                          <div className="category-header">
                            ...
                            <h6 className="fw-bold mb-1 text-info">
                              योजनाएं ({filteredUniqueSchemes.length})
                            </h6>
                          </div>
                        ...
                        <div className="category-section">
                          <div className="category-header">
                            ...
                            <h6 className="fw-bold mb-1 text-warning">
                              निवेश ({filteredUniqueInvestments.length})
                            </h6>
                          </div>
                          <div className="category-content">
                            {filteredUniqueInvestments.map(
                              (investment, index) => {
                                const tooltipData = getTooltipData(
                                  "investment",
                                  investment,
                        ...
                        <div className="category-section">
                          <div className="category-header">
                            ...
                            <h6 className="fw-bold mb-1 text-secondary">
                              घटक ({filteredUniqueComponents.length})
                            </h6>
                          </div>
                          <div className="category-content">
                            {filteredUniqueComponents.map(
                              (component, index) => {
                                const tooltipData = getTooltipData(
                                  "component",
                                  component,
                        ...
                        <div className="category-section">
                          <div className="category-header">
                            ...
                            <h6 className="fw-bold mb-1 text-dark">
                              सप्लायर ({filteredUniqueSources.length})
                            </h6>
                          </div>
                          <div className="category-content">
                    ...
                    {selectedSchemes.size === 0 &&
                      selectedComponents.size === 0 && (
                        <div className="text-center text-muted py-4">
                          ...
                          <small>
                            योजनाएं या घटक चुनें ताकि परिणाम दिखाई दें
                          </small>
                        </div>
                      )}
                  </div>
          ...
          >
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <Button
                ...
                title="सप्लायर फ़िल्टर डेटा Excel में निर्यात"
              >
                <FaFileExcel />
              </Button>
              <Button
                ...
                onClick={(e) => {
                  ...
                  exportSectionToPDF("सप्लायर_फ़िल्टर_डेटा", sourceFilterData);
                }}
                title="सप्लायर फ़िल्टर डेटा PDF में निर्यात"
              >
                <FaFilePdf />
              </Button>
            </div>
            ...
            <Card.Body>
              <Row>
                ...
                <Col md={6}>
                  <div className="selection-panel">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      <h6 className="fw-bold mb-0">सप्लायर चयन</h6>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                    ...
                    <div>
                      <h6 className="fw-bold mb-2 text-dark">
                        सभी सप्लायर ({uniqueSources.length})
                      </h6>
                      <div className="d-flex flex-wrap gap-1">
                        {uniqueSources.map((source, index) => {
                          ...
                          return (
                            <Badge
                              key={index}
                              bg={
                                selectedSources.has(source) ? "dark" : "light"
                              }
                              text={
                                selectedSources.has(source) ? "light" : "dark"
                              }
                              className="p-2 selectable-badge"
                              style={{ cursor: "pointer" }}
                              onClick={() => toggleSource(source)}
                              title={
                                selectedSources.has(source) ? "हटाएं" : "चुनें"
                              }
                              data-bs-toggle="tooltip"
                              data-bs-placement="top"
                            >
                ...
                <Col md={6}>
                  <div className="results-panel">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      <h6 className="fw-bold mb-0">सप्लायर आधारित परिणाम</h6>
                      <div className="text-end">
                        <small className="text-muted">
                      ...
                      <div className="bg-light p-2 rounded">
                        <div className="row text-center">
                          ...
                          <div className="col-4">
                            <div className="fw-bold text-success">
                              {formatCurrency(
                                sourceFilteredTotals.totalAllocated
                              )}
                            </div>
                            <small className="text-muted">आवंटित</small>
                          </div>
                          <div className="col-4">
                            <div className="fw-bold text-warning">
                              {formatCurrency(
                                sourceFilteredTotals.totalUpdated
                              )}
                            </div>
                            <small className="text-muted">बेचा गया</small>
                          </div>
                      ...
                      {sourceFilteredUniqueSchemes.length > 0 && (
                        <div className="category-section">
                          <div className="category-header">
                            <FaPiggyBank className="me-1 text-info" />
                            <h6 className="fw-bold mb-1 text-info">
                              योजनाएं ({sourceFilteredUniqueSchemes.length})
                            </h6>
                          </div>
                          <div className="category-content">
                            {sourceFilteredUniqueSchemes.map(
                              (scheme, index) => {
                                const tooltipData = getTooltipData(
                                  "scheme",
                                  scheme,
                      ...
                      {sourceFilteredUniqueInvestments.length > 0 && (
                        <div className="category-section">
                          <div className="category-header">
                            <FaPuzzlePiece className="me-1 text-warning" />
                            <h6 className="fw-bold mb-1 text-warning">
                              निवेश ({sourceFilteredUniqueInvestments.length})
                            </h6>
                          </div>
                          <div className="category-content">
                            {sourceFilteredUniqueInvestments.map(
                              (investment, index) => {
                                const tooltipData = getTooltipData(
                                  "investment",
                                  investment,
                      ...
                      {sourceFilteredUniqueComponents.length > 0 && (
                        <div className="category-section">
                          <div className="category-header">
                            <FaLayerGroup className="me-1 text-secondary" />
                            <h6 className="fw-bold mb-1 text-secondary">
                              घटक ({sourceFilteredUniqueComponents.length})
                            </h6>
                          </div>
                          <div className="category-content">
                            {sourceFilteredUniqueComponents.map(
                              (component, index) => {
                                const tooltipData = getTooltipData(
                                  "component",
                                  component,
                    ...
                    {selectedSources.size === 0 && (
                      <div className="text-center text-muted py-4">
                        ...
                        <div>कोई सप्लायर नहीं चुना गया</div>
                        <small>सप्लायर चुनें ताकि परिणाम दिखाई दें</small>
                      </div>
                    )}
                  </div>
                </Col>
            ...
            <Card.Header className=" d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const kendraSummaryData = [
                      ["केंद्र अनुसार सारांश", ""],
                      [
                        "केंद्र नाम",
                        "रिकॉर्ड संख्या",
                        "आवंटित राशि",
                    ...
                    Object.entries(centerSummaries).forEach(
                      ([kendraName, summary]) => {
                        kendraSummaryData.push([
                          kendraName,
                          summary.recordCount,
                          formatCurrency(summary.totalAllocated),
                          formatCurrency(summary.totalUpdated),
                    ...
                    const wb = XLSX.utils.book_new();
                    const ws = XLSX.utils.aoa_to_sheet(kendraSummaryData);
                  ...
                  title="केंद्र सारांश Excel में निर्यात"
                >
                  <FaFileExcel />
                </Button>
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const kendraSummaryData = Object.entries(
                      centerSummaries
                    ).map(([kendraName, summary]) => ({
                      केंद्र: kendraName,
                      रिकॉर्ड: summary.recordCount,
                      आवंटित: formatCurrency(summary.totalAllocated),
                      वितरण: formatCurrency(summary.totalUpdated),
                      शेष: formatCurrency(summary.totalRemaining),
                      प्रतिशत: `${summary.distributionPercentage}%`,
                    }));
                    exportSectionToPDF("केंद्र सारांश", kendraSummaryData);
                  }}
                  title="केंद्र सारांश PDF में निर्यात"
                >
                  <FaFilePdf />
                </Button>
              </div>
                      ...
                      <Row className="g-2 mb-3">
                        ...
                        <Col md={3}>
                          ...
                          >
                            ...
                            <h6
                              style={{ color: "#455a64", fontWeight: "bold" }}
                            >
                              {summary.distributionPercentage}%
                            </h6>
                          </div>
          ...
          <Card className="mb-3">
              ...
              <div className="d-flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    XLSX.utils.book_append_sheet(wb, ws, "तुलनात्मक सारांश");
                    XLSX.writeFile(
                      wb,
                      `तुलनात्मक_सारांश_${
                        new Date().toISOString().split("T")[0]
                  ...
                  title="तुलनात्मक सारांश Excel में निर्यात"
                >
                  <FaFileExcel />
                </Button>
                <Button
                  ...
                  onClick={() => {
                    const comparisonData = [
                      {
                        केंद्र: "कुल तुलना",
                        रिकॉर्ड: comparisonSummary.totalRecords,
                        आवंटित: formatCurrency(
                          comparisonSummary.totalAllocated
                        ),
                        वितरण: formatCurrency(comparisonSummary.totalUpdated),
                        शेष: formatCurrency(comparisonSummary.totalRemaining),
                        प्रतिशत: `${comparisonSummary.overallDistributionPercentage}%`,
                      },
                    ];
                    ...
                    exportSectionToPDF("तुलनात्मक सारांश", comparisonData);
                  }}
                  title="तुलनात्मक सारांश PDF में निर्यात"
                >
                  <FaFilePdf />
                </Button>
              </div>
            ...
            <Card.Body>
              <div className="text-center mb-4">
                <h5 className="text-primary fw-bold">केंद्रों के बीच तुलना</h5>
                <p className="text-muted">
                  सभी केंद्रों का समग्र विश्लेषण और तुलना
                </p>
              </div>
              ...
              <Row className="g-3 mb-4">
                <Col md={3}>
                  <div
                    className="text-center summary-card border rounded p-3"
                    style={{ backgroundColor: "#e3f2fd" }}
                  >
                    ...
                    <small className="text-muted">कुल केंद्र</small>
                  </div>
                </Col>
                <Col md={3}>
                  <div
                    ...
                    style={{ backgroundColor: "#f3e5f5" }}
                  >
                    <FaList size={24} className="text-info mb-2" />
                    <h4 className="text-info mb-1">
                      {comparisonSummary.totalRecords}
                    </h4>
                    <small className="text-muted">कुल रिकॉर्ड</small>
                  </div>
                </Col>
                <Col md={3}>
                  <div
                    ...
                    style={{ backgroundColor: "#fff3e0" }}
                  >
                    <FaPiggyBank size={24} className="text-success mb-2" />
                    <h4 className="text-success mb-1">
                      {formatCurrency(comparisonSummary.totalAllocated)}
                    </h4>
                    <small className="text-muted">कुल आवंटित</small>
                  </div>
                </Col>
                <Col md={3}>
                  <div
                    ...
                    style={{ backgroundColor: "#e8f5e9" }}
                  >
                    <FaChartBar size={24} className="text-warning mb-2" />
                    <h4 className="text-warning mb-1">
                      {comparisonSummary.overallDistributionPercentage}%
                    </h4>
                    <small className="text-muted">औसत वितरण %</small>
                  </div>
                </Col>
              </Row>
                ...
                <Table striped bordered hover className="comparison-table">
                  ...
                  <tbody>
                    {Object.entries(centerSummaries).map(
                      ([kendraName, summary]) => (
                        <tr key={kendraName}>
                          <td className="fw-bold">{kendraName}</td>
                          <td className="text-center">{summary.recordCount}</td>
                          ...
                          <td className="text-center">
                            <Badge
                              bg={
                                summary.distributionPercentage > 50
                                  ? "success"
                                  : summary.distributionPercentage > 25
                                  ? "warning"
                                  : "danger"
                              }
                            >
                          ...
                          <td className="text-center">
                            ...
                            >
                              {summary.totalRemaining > 0 ? "सक्रिय" : "पूर्ण"}
                            </Badge>
                          </td>
                        </tr>
                      )
                ...
                <Row>
                  <Col md={6}>
                    <div className="p-3 border rounded bg-light">
                      <h6 className="text-success">सर्वाधिक वितरण</h6>
                      {(() => {
                        const bestPerformer = Object.entries(
                          centerSummaries
                        ).reduce(
                          (best, [name, summary]) =>
                            summary.distributionPercentage > best.percentage
                              ? {
                                  name,
                                  percentage: summary.distributionPercentage,
                                }
                              : best,
                          { name: "", percentage: 0 }
                        );
                  ...
                  <Col md={6}>
                    <div className="p-3 border rounded bg-light">
                      <h6 className="text-warning">न्यूनतम वितरण</h6>
                      {(() => {
                        const worstPerformer = Object.entries(
                          centerSummaries
                        ).reduce(
                          (worst, [name, summary]) =>
                            summary.distributionPercentage < worst.percentage
                              ? {
                                  name,
                                  percentage: summary.distributionPercentage,
                                }
                        ...
                        return (
                          <p className="mb-0">
                            <strong>{worstPerformer.name}</strong> (
