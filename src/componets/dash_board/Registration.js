    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Skip header row and parse data according to the specified column sequence
          const dataRows = jsonData.slice(1);
          const headers = jsonData[0];
          console.log('Excel headers:', headers);
          console.log('First data row:', dataRows[0]);

          // Create a mapping from header names to column indices (trimmed and lowercased for robustness)
          const headerMapping = {};
          ...
          const payloads = dataRows.map(row => ({
            center_name: row[headerMapping['केंद्र का नाम']] || row[headerMapping['center_name']] || row[headerMapping['केंद्र का नाम'.toLowerCase()]] || row[headerMapping['center_name'.toLowerCase()]] || '',
            component: row[headerMapping['घटक']] || row[headerMapping['component']] || row[headerMapping['घटक'.toLowerCase()]] || row[headerMapping['component'.toLowerCase()]] || '',
            investment_name: row[headerMapping['निवेश का नाम']] || row[headerMapping['investment_name']] || row[headerMapping['निवेश का नाम'.toLowerCase()]] || row[headerMapping['investment_name'.toLowerCase()]] || '',
            sub_investment_name: row[headerMapping['उप-निवेश का नाम']] || row[headerMapping['sub_investment_name']] || row[headerMapping['उप-निवेश का नाम'.toLowerCase()]] || row[headerMapping['sub_investment_name'.toLowerCase()]] || '',
            unit: row[headerMapping['इकाई']] || row[headerMapping['unit']] || row[headerMapping['इकाई'.toLowerCase()]] || row[headerMapping['unit'.toLowerCase()]] || '',
            allocated_quantity: parseInt(row[headerMapping['आवंटित मात्रा']] || row[headerMapping['allocated_quantity']] || row[headerMapping['आवंटित मात्रा'.toLowerCase()]] || row[headerMapping['allocated_quantity'.toLowerCase()]] || 0),
            rate: parseFloat(row[headerMapping['दर']] || row[headerMapping['rate']] || row[headerMapping['दर'.toLowerCase()]] || row[headerMapping['rate'.toLowerCase()]] || 0),
            source_of_receipt: row[headerMapping['प्राप्ति का स्रोत']] || row[headerMapping['source_of_receipt']] || row[headerMapping['प्राप्ति का स्रोत'.toLowerCase()]] || row[headerMapping['source_of_receipt'.toLowerCase()]] || '',
            scheme_name: row[headerMapping['योजना का नाम']] || row[headerMapping['scheme_name']] || row[headerMapping['योजना का नाम'.toLowerCase()]] || row[headerMapping['scheme_name'.toLowerCase()]] || '',
            vikas_khand_name: row[headerMapping['विकास खंड का नाम']] || row[headerMapping['vikas_khand_name']] || row[headerMapping['विकास खंड का नाम'.toLowerCase()]] || row[headerMapping['vikas_khand_name'.toLowerCase()]] || '',
            vidhan_sabha_name: row[headerMapping['विधानसभा का नाम']] || row[headerMapping['vidhan_sabha_name']] || row[headerMapping['विधानसभा का नाम'.toLowerCase()]] || row[headerMapping['vidhan_sabha_name'.toLowerCase()]] || ''
          }));
          console.log('Parsed payloads:', payloads.slice(0, 3)); // Log first 3 payloads

          let successfulUploads = 0;
          const failedItems = [];

          console.log('Starting validation and upload for', payloads.length, 'items');

          for (let i = 0; i < payloads.length; i++) {
            try {
              const payload = payloads[i];
              console.log(`Validating item ${i + 1}:`, payload);
              console.log(`Item ${i + 1} raw values:`, {
                center_name: payload.center_name,
                component: payload.component,
                investment_name: payload.investment_name,
                unit: payload.unit,
                allocated_quantity: payload.allocated_quantity + ' (isNaN: ' + isNaN(payload.allocated_quantity) + ')',
                rate: payload.rate + ' (isNaN: ' + isNaN(payload.rate) + ')',
                source_of_receipt: payload.source_of_receipt,
                scheme_name: payload.scheme_name
              });

              // Validate only truly required fields
              // Optional fields: sub_investment_name, vikas_khand_name, vidhan_sabha_name
              if (!payload.center_name || !payload.component || !payload.investment_name ||
                  !payload.unit || isNaN(payload.allocated_quantity) || isNaN(payload.rate) ||
                  !payload.source_of_receipt || !payload.scheme_name) {
                console.log(`Item ${i + 1} failed validation:`, {
                  center_name: !!payload.center_name,
                  component: !!payload.component,
                  investment_name: !!payload.investment_name,
                  unit: !!payload.unit,
                  allocated_quantity: !isNaN(payload.allocated_quantity),
                  rate: !isNaN(payload.rate),
                  source_of_receipt: !!payload.source_of_receipt,
                  scheme_name: !!payload.scheme_name
                });
                failedItems.push({ index: i + 1, reason: 'Missing required fields' });
                continue;
              }

              console.log(`Sending API request for item ${i + 1}:`, payload);
              const response = await axios.post(BILLING_API_URL, payload);
              console.log(`API response for item ${i + 1}:`, response.status, response.data);

              if (response.status === 200 || response.status === 201) {
                setBillingItems(prev => [payload, ...prev]);
                successfulUploads++;
              } else {
                failedItems.push({ index: i + 1, reason: `Server returned status ${response.status}` });
              }
            } catch (error) {
              ...
              failedItems.push({ index: i + 1, reason: errorMessage });
            }
          }
          ...
          if (failedItems.length > 0) {
            setApiError(`${successfulUploads} items uploaded successfully, ${failedItems.length} items failed. Check console for details.`);
