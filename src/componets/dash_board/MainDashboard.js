...
const MainDashboard = () => {
  ...
  const handleViewSelectedCenters = () => {
    ...
    switch (selectedFilter) {
      case "kendra":
        ...
        allOptions = [...new Set(billingData.map((item) => item.center_name))]
          .filter(Boolean)
          .sort();
        // Filter items for all selected centers
        filteredItems = billingData.filter((item) =>
          selectedCenters.includes(item.center_name)
        );
        break;
      ...
      case "vidhanSabha":
        ...
        allOptions = [
          ...
          .filter(Boolean)
          .sort();
        // Filter items for all selected vidhan sabhas
        filteredItems = billingData.filter((item) =>
          selectedCenters.includes(item.vidhan_sabha_name)
        );
        break;
      ...
      case "vikasKhand":
        ...
        allOptions = [
          ...
          .filter(Boolean)
          .sort();
        // Filter items for all selected vikas khand
        filteredItems = billingData.filter((item) =>
          selectedCenters.includes(item.vikas_khand_name)
        );
        break;
    ...
    const groupData = {
      ...
      group_field: groupField,
      items: filteredItems, // Pass filtered items for selected centers
      allOptions: allOptions,
  ...
  const handleDrop = (e, targetItem) => {
    ...
    const dropTarget = e.currentTarget.closest(".all-centers")
      ? "all"
      : "selected";
    ...
    if (dropTarget === "selected") {
      ...
      if (!allCenters.includes(draggedItem)) {
        ...
        setSelectedCenters(
          selectedCenters.filter((item) => item !== draggedItem)
        );
      }
    }
  };
