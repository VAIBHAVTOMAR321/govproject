...
const generateVikasKhandSummary = (data) => {
  ...
  data.forEach((item) => {
    ...
    if (!groupedByVikasKhand[vikasKhand][center])
      groupedByVikasKhand[vikasKhand][center] = {};
  ...
  return Object.entries(groupedByVikasKhand).map(
    ([vikasKhand, centersObj]) => ({
      vikasKhand,
      centers: Object.entries(centersObj).map(([centerName, schemesObj]) => ({
        centerName,
        schemes: Object.entries(schemesObj).map(([schemeName, schemeObj]) => ({
          schemeName,
          investments: Object.entries(schemeObj.investmentsMap || {}).map(
            ([investmentName, subSet]) => ({
              investmentName,
...
const MainDashboard = () => {
  ...
  const [summaryTotalBreakdownColumn, setSummaryTotalBreakdownColumn] =
    useState("");

  // New: numeric display selector state - controls which hierarchy level numeric columns display aggregates for
  const [numericDisplayLevel, setNumericDisplayLevel] = useState(
    "sub_investment_name",
  );
  ...
  const [showAllocationPreviewModal, setShowAllocationPreviewModal] =
    useState(false);
  const [allocationPreviewType, setAllocationPreviewType] = useState("pdf");
  const [allocationPreviewContent, setAllocationPreviewContent] =
    useState(null);
  const [currentPreviewTableIndex, setCurrentPreviewTableIndex] =
    useState(null);
  ...
  const [vidhanSabhaGrouping, setVidhanSabhaGrouping] =
    useState("vidhan_sabha_name");

  // State for Vidhan Sabha table column type
  const [vidhanSabhaColumnType, setVidhanSabhaColumnType] =
    useState("investment_name");
  ...
  const generateTotalBreakdown = (data, groupByColumn) => {
    ...
    data.forEach((item) => {
      ...
      grouped[groupKey].investments[invName].totals.amount_of_farmer_share +=
        farmerShare;
      grouped[groupKey].investments[invName].totals.amount_of_subsidy +=
        subsidy;
      grouped[groupKey].investments[invName].totals.total_amount += total;

      // Add to sub-investment totals
      grouped[groupKey].investments[invName].subInvestments[
        subInvName
      ].totals.allocated_quantity += qty;
      grouped[groupKey].investments[invName].subInvestments[
        subInvName
      ].totals.amount_of_farmer_share += farmerShare;
      grouped[groupKey].investments[invName].subInvestments[
        subInvName
      ].totals.amount_of_subsidy += subsidy;
      grouped[groupKey].investments[invName].subInvestments[
        subInvName
      ...
      if (
        ...
        grouped[groupKey].investments[invName].subInvestments[subInvName].unit =
          unit;
      }
    });
  ...
  const convertToHierarchy = (rows, startLevel = "vidhan_sabha_name") => {
      ...
      for (let i = startIndex; i < levels.length; i++) {
        ...
        if (isLast) {
          ...
          node.subInvestments.push({
            ...
            unit: r.unit || "",
            allocated_quantity:
              r.allocated_quantity !== undefined &&
              r.allocated_quantity !== null &&
              r.allocated_quantity !== ""
                ? parseFloat(r.allocated_quantity)
                : undefined,
            amount_of_farmer_share:
              r.amount_of_farmer_share !== undefined &&
              r.amount_of_farmer_share !== null &&
              r.amount_of_farmer_share !== ""
                ? parseFloat(r.amount_of_farmer_share)
                : undefined,
            amount_of_subsidy:
              r.amount_of_subsidy !== undefined &&
              r.amount_of_subsidy !== null &&
              r.amount_of_subsidy !== ""
                ? parseFloat(r.amount_of_subsidy)
                : undefined,
            total_amount:
    ...
    const computeAggregates = (node) => {
      ...
      if (Array.isArray(node.subInvestments)) {
        const sums = node.subInvestments.reduce(
          (acc, s) => {
            acc.allocated_quantity += parseFloat(s.allocated_quantity) || 0;
            acc.amount_of_farmer_share +=
              parseFloat(s.amount_of_farmer_share) || 0;
    ...
    if (additionalTables.length > 0) {
      ...
      additionalTables.forEach((table, index) => {
        ...
        if (table.isAllocationTable && !newAdditionalColumnFilters[index]) {
          const dynamicCols = table.columns
            .slice(1)
            .filter(
              (col) =>
                !col.endsWith("_dar") &&
  ...
  const schemeChartData = useMemo(() => {
    const dataForHierarchy =
      filteredTableData && filteredTableData.length > 0
        ? filteredTableData
        : tableData;
    const headerFilteredData = applySummaryHeaderFilters(
      dataForHierarchy || [],
    );
  ...
  const combinedTableData = useMemo(() => {
    ...
    headerFilteredData.forEach((item) => {
      ...
      if (!investmentSchemeData[investment])
        investmentSchemeData[investment] = {};
      if (!investmentSchemeData[investment][scheme])
        investmentSchemeData[investment][scheme] = {
          subsidy: 0,
          farmerShare: 0,
          total: 0,
          quantity: 0,
    ...
    investments.forEach((inv) => {
      Object.keys(investmentSchemeData[inv]).forEach((sch) =>
        allSchemes.add(sch),
      );
    });
    ...
    const quantities = {};
    investments.forEach((inv) => {
      totals[inv] = schemes.reduce(
        (sum, sch) =>
          sum +
          ((investmentSchemeData[inv][sch] &&
            investmentSchemeData[inv][sch].subsidy) ||
            0),
        0,
      );
      quantities[inv] = schemes.reduce(
        (sum, sch) =>
          sum +
          ((investmentSchemeData[inv][sch] &&
            investmentSchemeData[inv][sch].quantity) ||
            0),
        0,
      );
    });
    const grandTotal = Object.values(totals).reduce((sum, val) => sum + val, 0);
    const grandQuantity = Object.values(quantities).reduce(
      (sum, val) => sum + val,
      0,
    );
  ...
  const subCombinedTableData = useMemo(() => {
    ...
    headerFilteredData.forEach((item) => {
      ...
      const total = parseFloat(item.total_amount) || 0;
      const qty = parseFloat(item.allocated_quantity) || 0;
      if (!subInvestmentSchemeData[subInvestment])
        subInvestmentSchemeData[subInvestment] = {};
      if (!subInvestmentSchemeData[subInvestment][scheme])
        subInvestmentSchemeData[subInvestment][scheme] = {
          subsidy: 0,
          farmerShare: 0,
          total: 0,
          quantity: 0,
    ...
    subInvestments.forEach((subInv) => {
      Object.keys(subInvestmentSchemeData[subInv]).forEach((sch) =>
        allSchemes.add(sch),
      );
    });
    const schemes = Array.from(allSchemes).sort();
    ...
    const quantities = {};
    subInvestments.forEach((subInv) => {
      totals[subInv] = schemes.reduce(
        (sum, sch) =>
          sum + ((data[subInv][sch] && data[subInv][sch].subsidy) || 0),
        0,
      );
      quantities[subInv] = schemes.reduce(
        (sum, sch) =>
          sum + ((data[subInv][sch] && data[subInv][sch].quantity) || 0),
        0,
      );
    });
    const grandTotal = Object.values(totals).reduce((sum, val) => sum + val, 0);
    ...
    setSummaryColumnValueFilters((prev) => {
      ...
      for (const col of summaryKeys) {
        ...
        if (col === currentFilter.column) {
          ...
          const checkedValuesFromFilter = Object.keys(
          ...
          next[col] = checkedValuesFromFilter.filter((v) =>
            allValuesForFirstColumn.includes(v),
          );
        } else {
    ...
    rowSpecificData.forEach((item) => {
      ...
      if (investmentName && subInvestmentName) {
        ...
        if (!groupedData[investmentName][subInvestmentName]) {
          groupedData[investmentName][subInvestmentName] = {
            matra: 0,
            unit: "",
          };
        }
    ...
    return Object.entries(groupedData).map(
      ([investmentName, subInvestments]) => {
        ...
        return {
          ...
          subInvestments: Object.entries(subInvestments).map(
            ([name, info]) => ({
              name,
              matra: info.matra.toFixed(2),
  ...
  const getSubInvestmentGroupedByInvestmentWithUnit = (rowValue) => {
    ...
    rowSpecificData.forEach((item) => {
      ...
      if (!groupedData[investmentName].subs[subInvestmentName]) {
        groupedData[investmentName].subs[subInvestmentName] = {
          matra: 0,
          unit: "",
        };
      }
      ...
      if (!groupedData[investmentName].subs[subInvestmentName].unit) {
        ...
        if (uSub)
          groupedData[investmentName].subs[subInvestmentName].unit = uSub;
      }
    });
  ...
  const getSchemeInvestmentHierarchyWithUnit = (rowValue) => {
    ...
    rowSpecificData.forEach((item) => {
      ...
      if (!grouped[scheme][investment].subs[subInvestment]) {
        grouped[scheme][investment].subs[subInvestment] = {
          matra: 0,
          unit: "",
        };
      }
  ...
  const handleAdditionalTableColumnToggle = (tableIndex, column) => {
    setTableColumnFilters((prev) => {
      ...
      if (currentSelected.includes(column)) {
        ...
        const prevPos = (removedColumnPositions.additional || {})[tableIndex]?.[
          column
        ];
        if (prevPos !== undefined && prevPos >= 0) {
          newSelected = [...currentSelected];
  ...
  useEffect(() => {
    try {
      const hierarchy = convertToHierarchy(
        filteredTableData || tableData || [],
        selectedHierarchyLevel || "vidhan_sabha_name",
      );
  ...
  const handleGenerateReport = () => {
    ...
    const uniqueValues = [
      ...new Set(
        filteredByDate.map((item) => item[reportType]).filter(Boolean),
      ),
    ];
  ...
  const confirmReportGeneration = () => {
    ...
    setSelectedItem({
      column: reportType,
      value: selectedReportValues.join(", "),
    });
  ...
  useEffect(() => {
    if (view === "detail" && filterStack.length > 0) {
      ...
      if (filteredData.length > 0) {
        const currentFilter = filterStack[filterStack.length - 1];
        const availableColumns = Object.keys(columnDefs).filter(
          (col) => col !== currentFilter.column && !columnDefs[col].hidden,
        );
  ...
  const generateSummary = (data, column) => {
          ...
          Object.keys(columnDefs)
            ...
            .map((col) => {
              ...
              if (column === "scheme_name" && col === "investment_name") {
                const investments = [
                  ...new Set(
                    dataForValue.map((item) => item[col]).filter(Boolean),
                  ),
                ];
                return [
    ...
    const filteredVisibleColumns = visibleColumns.filter(
      (col) => col !== translations.sourceOfReceipt,
    );
  ...
  const generateVidhanSabhaInvestmentTable = (
    data,
    grouping = "vidhan_sabha_name",
    columnType = "investment_name",
    rashiType = "कुल राशि",
  ) => {
    if (!data || !Array.isArray(data) || data.length === 0) return null;

    const groupLabel =
      grouping === "center_name"
        ? "केंद्र"
        : grouping === "vidhan_sabha_name"
          ? "विधानसभा"
          : "विकास खंड";
    const groups = [
      ...new Set(data.map((item) => item[grouping]).filter(Boolean)),
    ].sort();

    const dynamicColumns = [
      ...new Set(
        data
          .map((item) => `${item.scheme_name} - ${item[columnType]}`)
          .filter(Boolean),
      ),
    ].sort();
    ...
    dynamicColumns.forEach((col) => {
      ...
      const matchingItems = data.filter(
        (item) => item.scheme_name === scheme && item[columnType] === val,
      );
      let unitValue = "";
    ...
    const rows = groups.map((group) => {
      ...
      dynamicColumns.forEach((col) => {
        ...
        const amount = filteredData.reduce(
          (acc, item) => acc + (parseFloat(item[amountField]) || 0),
          0,
        );
      ...
      row["अनुदान राशि (रु0)"] = groupData
        ...
        .toFixed(2);
      row["कुल राशि"] = groupData
        .reduce((acc, item) => acc + (parseFloat(item.total_amount) || 0), 0)
        .toFixed(2);
      return row;
    });
    ...
    dynamicColumns.forEach((col) => {
      totalRow[`${col}_मात्रा`] = rows
        .reduce((sum, row) => sum + (parseFloat(row[`${col}_मात्रा`]) || 0), 0)
        .toFixed(2);
      totalRow[`${col}_राशि`] = rows
  ...
  useEffect(() => {
    if (isFilterApplied && filteredTableData && filteredTableData.length > 0) {
      const schemeFilteredData =
        vidhanSabhaSchemeFilter.length > 0
          ? filteredTableData.filter((item) =>
              vidhanSabhaSchemeFilter.includes(item.scheme_name),
            )
          : filteredTableData;
      const table = generateVidhanSabhaInvestmentTable(
        schemeFilteredData,
        vidhanSabhaGrouping,
        vidhanSabhaColumnType,
        vidhanSabhaRashiType,
      );
      if (table) {
        const groupLabel =
          vidhanSabhaGrouping === "center_name"
            ? "केंद्र"
            : vidhanSabhaGrouping === "vidhan_sabha_name"
              ? "विधानसभा"
              : "विकास खंड";
        const columnLabel =
        ...
        setAdditionalTables((prev) => {
          const existingIndex = prev.findIndex(
            (t) => t.type === "vidhanSabhaInvestment",
          );
      ...
      setAdditionalTables((prev) =>
        prev.filter((t) => t.type !== "vidhanSabhaInvestment"),
      );
    }
  }, [
    isFilterApplied,
  ...
  const generateChartData = () => {
    ...
    let baseData =
      currentDisplayedTable.data && currentDisplayedTable.data.length > 0
    ...
    dataToVisualize.forEach((item) => {
      ...
      if (value !== undefined && value !== null && value !== "") {
        ...
        const matra = item.allocated_quantity
          ? parseFloat(item.allocated_quantity)
          : 0;
        const dar = item.rate ? parseFloat(item.rate) : 0;
        const farmerShare = item.amount_of_farmer_share
          ? parseFloat(item.amount_of_farmer_share)
          : 0;
        const subsidy = item.amount_of_subsidy
        ...
        detailsMap[value].maxMatra = Math.max(
          detailsMap[value].maxMatra,
          matra,
        );
        detailsMap[value].minMatra = Math.min(
          detailsMap[value].minMatra,
          matra,
        );
          ...
          label: function (context) {
            ...
            if (metadata && metadata.detailsMap && metadata.detailsMap[label]) {
              ...
              const avgFarmerShare = (
                details.totalFarmerShare / details.count
              ).toFixed(2);
              const avgSubsidy = (details.totalSubsidy / details.count).toFixed(
                2,
              );
              const avgTotalAmount = (
                details.totalAmount / details.count
              ).toFixed(2);
  ...
  const getCurrentTableData = () => {
    if (view === "main") {
      ...
      const formatBreakdownForExport = (breakdown, valueField) => {
        ...
        breakdown.forEach((group) => {
          ...
          const groupValue =
            valueField === "name"
              ? group.groupName
              : (group.totals[valueField] || 0).toFixed(2);
      ...
      checkedValues.forEach((checkedValue) => {
        ...
        const breakdown = generateTotalBreakdown(
          filteredRowData,
        ...
        tableColumnOrder.forEach((col) => {
          if (
            ...
            if (col === summaryTotalBreakdownColumn) {
              row[columnDefs[col].label] = formatBreakdownForExport(
                breakdown,
                "name",
              );
              ...
              const uniqueValues = [
                ...new Set(
                  filteredRowData.map((item) => item[col]).filter(Boolean),
                ),
              ];
        ...
        const rowTotals = {
          allocated_quantity: filteredRowData.reduce((sum, item) => {
            const qtyVal =
              typeof item.allocated_quantity === "string" &&
              item.allocated_quantity.includes(" / ")
                ? parseFloat(item.allocated_quantity.split(" / ")[0]) || 0
                : parseFloat(item.allocated_quantity) || 0;
            return sum + qtyVal;
          }, 0),
          amount_of_farmer_share: filteredRowData.reduce(
            (sum, item) => sum + (parseFloat(item.amount_of_farmer_share) || 0),
            0,
          ),
          amount_of_subsidy: filteredRowData.reduce(
            (sum, item) => sum + (parseFloat(item.amount_of_subsidy) || 0),
            0,
          ),
          total_amount: filteredRowData.reduce(
      ...
      const totalBreakdown = generateTotalBreakdown(
        totalRowData,
        summaryTotalBreakdownColumn,
      );
      ...
      const grandTotals = {
        ...
        amount_of_farmer_share: totalRowData.reduce(
          (sum, item) => sum + (parseFloat(item.amount_of_farmer_share) || 0),
          0,
        ),
        amount_of_subsidy: totalRowData.reduce(
          (sum, item) => sum + (parseFloat(item.amount_of_subsidy) || 0),
          0,
        ),
        total_amount: totalRowData.reduce(
          (sum, item) => sum + (parseFloat(item.total_amount) || 0),
          0,
        ),
      };
      ...
      tableColumnOrder.forEach((col) => {
        if (
          ...
          if (col === summaryTotalBreakdownColumn) {
            ...
            totalRow[columnDefs[col].label] =
              headerLine +
              "\n" +
              formatBreakdownForExport(totalBreakdown, "name");
          } else if (mainSummaryExpandedColumns[col]) {
            const uniqueValues = [
              ...new Set(totalRowData.map((item) => item[col]).filter(Boolean)),
            ];
      ...
      const visibleColumns =
        tableColumnFilters.detail.length > 0
          ...
          : Object.keys(columnDefs)
              .filter(
                (col) =>
                  col !== currentFilter.column && !columnDefs[col].hidden,
              )
              .filter(
                (col) =>
                  col !== "allocated_quantity" &&
  ...
  const addAdditionalTableToExport = (table, type, index) => {
    ...
    const showDar =
      table.isAllocationTable && allocationTableToggles[index]?.dar;
    const showMatra =
      table.isAllocationTable && allocationTableToggles[index]?.matra;
    const showIkai =
      table.isAllocationTable && allocationTableToggles[index]?.ikai;
      ...
      if (!table.isAllocationTable) {
        ...
        visibleColumns.forEach((col) => {
          ...
          if (darVal === undefined) {
            ...
            darVal =
              String(darVal).split(" / ")[1] || String(darVal).split(" / ")[0];
          } else {
            darVal = String(darVal);
          }

          ikaiVal =
            ikaiVal !== undefined && ikaiVal !== null ? String(ikaiVal) : "";
          ...
          newRow[col] = formatAllocationValue(
            matraVal,
            ikaiVal,
            darVal,
            showMatra,
    ...
    const visibleDynamicColsExport = visibleColumns.filter(
      (c) =>
        c !== table.columns[0] &&
        !c.startsWith("कुल") &&
        c !== "आवंटित मात्रा" &&
        ...
        if (table.isAllocationTable) {
          const matraTotal = filteredTableData
            .reduce(
              (sum, row) =>
                sum +
                visibleDynamicColsExport.reduce(
                  (s, c) => s + parseFloat(row[c] || 0),
                  0,
                ),
              0,
            )
          ...
          const ikaiTotal = filteredTableData
            .reduce(
              (sum, row) =>
                ...
                visibleDynamicColsExport.reduce(
                  (s, c) =>
                    s + parseFloat(row[`${c}_ikai`] || row[`${c}_unit`] || 0),
                  0,
                ),
              0,
            )
            .toFixed(2);
          const showDarFlag =
            allocationTableToggles[index]?.dar ?? table.showDar ?? false;
          const showMatraFlag =
            allocationTableToggles[index]?.matra ?? table.showMatra ?? false;
          const showIkaiFlag =
            allocationTableToggles[index]?.ikai ?? table.showIkai ?? false;
          totals[col] = formatAllocationValue(
            matraTotal,
            ikaiTotal,
            darTotal,
            showMatraFlag,
          ...
          totals[col] = filteredTableData
            .reduce(
              (sum, row) =>
                sum +
                visibleDynamicColsExport.reduce(
                  (s, c) => s + parseFloat(row[`${c}_farmer`] || 0),
                  0,
                ),
              0,
            )
          ...
          totals[col] = filteredTableData
            .reduce(
              (sum, row) =>
                sum +
                visibleDynamicColsExport.reduce(
                  (s, c) => s + parseFloat(row[`${c}_subsidy`] || 0),
                  0,
                ),
              0,
            )
  ...
  const transposeTableForRotation = (table) => {
    ...
    const rowHeaders =
      table.visibleTransposed && table.visibleTransposed.length > 0
        ? table.visibleTransposed.filter(
            (c) => c !== firstColLabel && c !== "कुल",
          )
        : sourceData
    ...
    dynamicCols.forEach((col) => {
      ...
      rowHeaders.forEach((rh) => {
        ...
        if (table.isAllocationTable) {
          ...
          const ikaiValRaw =
            resolveCellValue(dataRow, `${col}_ikai`) ||
            resolveCellValue(dataRow, `${col}_unit`) ||
            "";
          ...
          newRow[rh] = formatAllocationValue(
            matraVal,
            ikaiVal,
            darVal,
            table.showMatra,
      ...
      if (rowHeaders.length > 0) {
        if (table.isAllocationTable) {
          const matraTotal = rowHeaders
            ...
            .toFixed(2);
          const darTotal = rowHeaders
            .reduce((s, rh) => {
              const r = sourceData.find((rr) => rr[firstColLabel] === rh);
              return s + parseFloat(resolveCellValue(r, `${col}_dar`) || 0);
            }, 0)
            .toFixed(2);
          const ikaiTotal = rowHeaders
            .reduce((s, rh) => {
              const r = sourceData.find((rr) => rr[firstColLabel] === rh);
              return (
                s +
                parseFloat(
                  resolveCellValue(r, `${col}_ikai`) ||
                    resolveCellValue(r, `${col}_unit`) ||
                    0,
                )
              );
            }, 0)
            .toFixed(2);
          newRow["कुल"] = formatAllocationValue(
            matraTotal,
            ikaiTotal,
            darTotal,
            table.showMatra,
        ...
        rowHeaders.forEach((rh) => {
          ...
          if (summaryCol === "आवंटित मात्रा") {
            const matraTotal = dynamicCols
              ...
              .toFixed(2);
            const darTotal = dynamicCols
              .reduce(
                (s, c) =>
                  s + parseFloat(resolveCellValue(dataRow, `${c}_dar`) || 0),
                0,
              )
              .toFixed(2);
            const ikaiTotal = dynamicCols
              .reduce(
                (s, c) =>
                  s +
                  parseFloat(
                    resolveCellValue(dataRow, `${c}_ikai`) ||
                      resolveCellValue(dataRow, `${c}_unit`) ||
                      0,
                  ),
                0,
              )
            ...
            newRow[rh] = dynamicCols
              .reduce(
                (s, c) =>
                  s + parseFloat(resolveCellValue(dataRow, `${c}_farmer`) || 0),
                0,
              )
              .toFixed(2);
            ...
            newRow[rh] = dynamicCols
              .reduce(
                (s, c) =>
                  s +
                  parseFloat(resolveCellValue(dataRow, `${c}_subsidy`) || 0),
                0,
              )
              .toFixed(2);
            ...
            newRow[rh] =
              resolveCellValue(dataRow, summaryCol) !== undefined
                ? String(resolveCellValue(dataRow, summaryCol))
                : "0";
          }
        });
          ...
          const matraTotal = rowHeaders
            .reduce((s, rh) => {
              ...
              return (
                s +
                dynamicCols.reduce(
                  (rs, c) => rs + parseFloat(resolveCellValue(r, c) || 0),
                  0,
                )
              );
            }, 0)
          ...
          const darTotal = rowHeaders
            .reduce((s, rh) => {
              ...
              return (
                ...
                dynamicCols.reduce(
                  (rs, c) =>
                    rs + parseFloat(resolveCellValue(r, `${c}_dar`) || 0),
                  0,
                )
              );
            }, 0)
          ...
          newRow["कुल"] = rowHeaders
            .reduce((s, rh) => {
              ...
              return (
                ...
                dynamicCols.reduce(
                  (rs, c) =>
                    rs + parseFloat(resolveCellValue(r, `${c}_farmer`) || 0),
                  0,
                )
              );
            }, 0)
            ...
            .reduce((s, rh) => {
              ...
              return (
                s +
                  dynamicCols.reduce(
                    (rs, c) =>
                      rs + parseFloat(resolveCellValue(r, `${c}_subsidy`) || 0),
                    0,
                  ),
                0
              );
    ...
    rowHeaders.forEach((rh) => {
      if (table.isAllocationTable) {
        const matraSum = dynamicCols
          .reduce((s, c) => {
            const r = sourceData.find((rr) => rr[firstColLabel] === rh);
            return s + parseFloat(resolveCellValue(r, c) || 0);
          }, 0)
          .toFixed(2);
        const darSum = dynamicCols
          .reduce((s, c) => {
            const r = sourceData.find((rr) => rr[firstColLabel] === rh);
    ...
    if (table.isAllocationTable) {
      const grandMatra = dynamicCols
        .reduce((s, c) => {
          return (
            s +
            rowHeaders.reduce((ss, rh) => {
              const r = sourceData.find((rr) => rr[firstColLabel] === rh);
    ...
    const hasAny =
      rowHeaders.length > 0 &&
      (dynamicCols.length > 0 || summaryColsToInclude.length > 0);
  ...
  const resolveCellValue = (row, col) => {
    ...
    for (const k of Object.keys(row)) {
      if (
        String(k).includes(normalizedCol) ||
        normalizedCol.includes(String(k))
      ) {
        return row[k] ?? "";
      }
    }
  ...
  const formatAllocationValue = (
    ...
    const m =
      matra === undefined || matra === null || String(matra).trim() === ""
        ? "0"
        : String(matra);
    const i =
      ikai === undefined || ikai === null || String(ikai).trim() === ""
        ? ""
        : String(ikai);
    const d =
      dar === undefined || dar === null || String(dar).trim() === ""
        ? "0"
        : String(dar);
            ...
            >
              ...
              >
                        ...
                        {displayTable.columns.map((col) => {
                          ...
                          if (
                            ...
                            if (colKey === "investment_name") {
                              const vals = getUniqueValuesWithMatra(
                                "investment_name",
                                null,
                              );
                              ...
                              const grouped =
                                getSubInvestmentGroupedByInvestment(null);
                              const formatted = [];
                ...
                {(() => {
                  ...
                  const isRotatedWithTotal =
                    displayTable.columns.includes("कुल");
  ...
  const generatePDF = () => {
    try {
      ...
      tablesForExport.pdf.forEach((table, index) => {
        ...
        dataToProcess.forEach((row, rowIndex) => {
          ...
          displayTable.columns.forEach((col) => {
            ...
            if (typeof cellValue === "string") {
              ...
              cellValue = cellValue.replace(
                /\*\*(.+?)\*\*/g,
                '<strong style="background-color: #e9ecef; padding: 2px 4px; border-radius: 3px;">$1</strong>',
              );
            }
        ...
        const hasKulAsColumn =
          displayTable.columns.includes("कुल") && table.isSummary;
  ...
  const exportOpenCollapsibleTablesToPDF = async () => {
    try {
      ...
      if (openCollapses.subInvestment) {
        const subInvestmentTable = document.getElementById(
          "subinvestment-table",
        );
  ...
  const flattenHierarchyWithRowspans = (
    hierarchy,
    visible,
    levels,
    numericLevel,
    ...
    const traverse = (nodes, levelIdx, nodesRef = [], path = []) => {
      nodes.forEach((node) => {
        ...
        if (levelIdx === levels.length - 1) {
          if (
            Array.isArray(node.subInvestments) &&
            node.subInvestments.length > 0
          ) {
    ...
    const rows = flat.map((r, idx) => {
      ...
      for (let li = 0; li < levels.length; li++) {
        ...
        const prevPrefix =
          idx > 0 ? flat[idx - 1].path.slice(0, li + 1).join("|") : null;
      ...
      const numericShownOnThisRow =
        numAggIndex >= 0
          ? idx === 0
            ? true
            : flat[idx - 1].path.slice(0, numAggIndex + 1).join("|") !==
              r.path.slice(0, numAggIndex + 1).join("|")
      ...
      if (visible.showAllocated) {
        cells.push({
          value:
            numericShownOnThisRow &&
            numAgg &&
            numAgg.allocated_quantity !== undefined
              ? numAgg.allocated_quantity
              : "",
          rowSpan: 1,
          isFirst: true,
        });
      ...
      if (visible.showFarmer) {
        cells.push({
          value:
            numericShownOnThisRow &&
            numAgg &&
            numAgg.amount_of_farmer_share !== undefined
              ? numAgg.amount_of_farmer_share
              : "",
          rowSpan: 1,
          isFirst: true,
          isCurrency: true,
      ...
      if (visible.showSubsidy) {
        cells.push({
          value:
            numericShownOnThisRow &&
            numAgg &&
            numAgg.amount_of_subsidy !== undefined
              ? numAgg.amount_of_subsidy
              : "",
          rowSpan: 1,
          isFirst: true,
          isCurrency: true,
      ...
      if (visible.showTotal) {
        cells.push({
          value:
            numericShownOnThisRow && numAgg && numAgg.total_amount !== undefined
              ? numAgg.total_amount
              : "",
          rowSpan: 1,
          isFirst: true,
    ...
    try {
      ...
      const selectedCol =
        filterStack.length > 0
          ? filterStack[filterStack.length - 1].column
          : "vidhan_sabha_name";
      ...
      const visible = {
        showVidhansabha:
          ...
          tableColumnFilters.summary.includes(
            columnDefs.vidhan_sabha_name.label,
          ),
        showVikasKhand:
          selIndex <= 1 &&
          tableColumnFilters.summary.includes(
            columnDefs.vikas_khand_name.label,
          ),
        showKendra:
          selIndex <= 2 &&
          tableColumnFilters.summary.includes(columnDefs.center_name.label),
        showYojana:
          selIndex <= 3 &&
          tableColumnFilters.summary.includes(columnDefs.scheme_name.label),
        showNivesh:
          selIndex <= 4 &&
          tableColumnFilters.summary.includes(columnDefs.investment_name.label),
        showUpNivesh:
          selIndex <= 5 &&
          tableColumnFilters.summary.includes(
      ...
      const rowsWithSpans = flattenHierarchyWithRowspans(
        hierarchy,
        visible,
        displayLevels,
        numericDisplayLevel || selectedHierarchyLevel,
      );

      // Calculate grand totals for footer
      const grandTotals = {
        allocated_quantity: headerFilteredData
          .reduce((sum, item) => {
            const qtyVal =
              typeof item.allocated_quantity === "string" &&
              item.allocated_quantity.includes(" / ")
          ...
          .toFixed(2),
        amount_of_farmer_share: headerFilteredData
          .reduce(
            (sum, item) => sum + (parseFloat(item.amount_of_farmer_share) || 0),
            0,
          )
          .toFixed(2),
        amount_of_subsidy: headerFilteredData
          .reduce(
            (sum, item) => sum + (parseFloat(item.amount_of_subsidy) || 0),
            0,
          )
          .toFixed(2),
        total_amount: headerFilteredData
          .reduce((sum, item) => sum + (parseFloat(item.total_amount) || 0), 0)
          .toFixed(2),
      };

      // Generate HTML rows with proper rowspans
      const htmlRows = rowsWithSpans
        .map((rowCells) => {
      ...
      const footerCells = [];
      footerCells.push(
        `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: Arial, sans-serif;">कुल:</td>`,
      );
      ...
      for (let i = 1; i < hierarchyColumns.length; i++) {
        ...
        const uniqueValues = [
          ...new Set(
            headerFilteredData.map((item) => item[col]).filter(Boolean),
          ),
        ];
        const displayValue = uniqueValues.join(", ");
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-family: Arial, sans-serif; font-size: 10px;">${displayValue}</td>`,
        );
      }
      ...
      if (visible.showIkai) {
        const uniqueUnits = [
          ...new Set(
            headerFilteredData.map((item) => item.unit).filter(Boolean),
          ),
        ];
        const ikaiFinalValue =
          uniqueUnits.length > 0 ? uniqueUnits.join(", ") : "";
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-family: Arial, sans-serif; font-size: 10px;">${ikaiFinalValue}</td>`,
        );
      }
      // Add numeric total cells
      if (visible.showAllocated) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: Arial, sans-serif;">${grandTotals.allocated_quantity}</td>`,
        );
      }
      if (visible.showFarmer) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: Arial, sans-serif;">₹${parseFloat(grandTotals.amount_of_farmer_share).toLocaleString()}</td>`,
        );
      }
      if (visible.showSubsidy) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: Arial, sans-serif;">₹${parseFloat(grandTotals.amount_of_subsidy).toLocaleString()}</td>`,
        );
      }
      if (visible.showTotal) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: Arial, sans-serif;">₹${parseFloat(grandTotals.total_amount).toLocaleString()}</td>`,
        );
      }
      ...
      try {
        ...
        if (imgHeight <= safePageHeight) {
          ...
          const safePageHeightPixels =
            (safePageHeight * canvas.width) / imgWidth;
        ...
        alert(
          "PDF generation failed. Please check browser console and try again.",
        );
      }
    } catch (error) {
  ...
  const previewDetailTablePDF = async () => {
    try {
      ...
      const footerCells = [];
      footerCells.push(
        `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">कुल:</td>`,
      );
      // Add unique values for hierarchy columns (except the first one which has "कुल:")
      const hierarchyColumns = [];
      if (visible.showVidhansabha) hierarchyColumns.push("vidhan_sabha_name");
      ...
      for (let i = 1; i < hierarchyColumns.length; i++) {
        ...
        const displayValue = uniqueValues.join(", ");
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-size: 10px;">${displayValue}</td>`,
        );
      }
      // Add ikai column cell with unique unit values
      if (visible.showIkai) {
        ...
        const ikaiFinalValue =
          uniqueUnits.length > 0 ? uniqueUnits.join(", ") : "";
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-size: 10px;">${ikaiFinalValue}</td>`,
        );
      }
      // Add numeric total cells
      if (visible.showAllocated) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">${grandTotals.allocated_quantity}</td>`,
        );
      }
      if (visible.showFarmer) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">₹${parseFloat(grandTotals.amount_of_farmer_share).toLocaleString()}</td>`,
        );
      }
      if (visible.showSubsidy) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">₹${parseFloat(grandTotals.amount_of_subsidy).toLocaleString()}</td>`,
        );
      }
      if (visible.showTotal) {
        footerCells.push(
          `<td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">₹${parseFloat(grandTotals.total_amount).toLocaleString()}</td>`,
        );
      }
      const footerRow = `<tr>${footerCells.join("")}</tr>`;
  ...
  const previewAllocationTablePDF = async (table, tableIndex) => {
    try {
      ...
      console.error("PDF preview error:", error);
      alert(
        "PDF पूर्वालोकन में त्रुटि हुई: " + (error.message || "अज्ञात त्रुटि"),
      );
    }
  };
  ...
  const previewAllocationTableExcel = async (table, tableIndex) => {
    try {
      ...
      console.error("Excel preview error:", error);
      alert(
        "Excel पूर्वालोकन में त्रुटि हुई: " +
          (error.message || "अज्ञात त्रुटि"),
      );
    }
  };
  ...
  const exportAllocationTableToPDF = async (table, tableIndex) => {
    try {
      ...
      console.error("PDF export error:", error);
      alert(
        "PDF निर्यात में त्रुटि हुई: " + (error.message || "अज्ञात त्रुटि"),
      );
    }
  };
  ...
  const exportAllocationTableToExcel = async (table, tableIndex) => {
    try {
      ...
      const sheetName = table.heading
        ? table.heading.length > 31
          ? table.heading.substring(0, 28) + "..."
          : table.heading
        : "Allocation Table";
      ...
      console.error("Excel export error:", error);
      alert(
        "Excel निर्यात में त्रुटि हुई: " + (error.message || "अज्ञात त्रुटि"),
      );
    }
  };
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (!tableEl) {
                              alert(
                                "Unable to find the rendered report table for export.",
                              );
                              return;
                            }
                            await exportTableToPDF(tableEl, "report.pdf", true);
  ...
  const generateDetailedBreakdownTable = (clickedValues, columnKey) => {
    ...
    if (clickedValues.length === 1) {
      ...
      showColumns.forEach((label) => {
        const uniqueValues = [
          ...new Set(newTableData.map((row) => row[label]).filter(Boolean)),
        ];
    ...
    newColumns.forEach((col) => {
      ...
      const uniqueValues = [
        ...new Set(newTableData.map((row) => row[col]).filter(Boolean)),
      ];
  ...
  const ReportModal = () => (
    <Modal show={showReportModal} onHide={() => setShowReportModal(false)}>
      ...
      <Modal.Body onClick={(e) => e.stopPropagation()}>
        <Form.Group>
          <Form.Label>
            {columnDefs[reportType]?.label || reportType} चुनें
          </Form.Label>
          <div onClick={(e) => e.stopPropagation()}>
            <Select
              ...
              options={[
                { value: "ALL", label: "सभी चुनें" },
                ...reportMultiSelectOptions.map((val) => ({
                  value: val,
                  label: val,
                })),
              ]}
              value={selectedReportValues.map((val) => ({
                value: val,
                label: val,
              }))}
              onChange={(selected) => {
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" onClick={confirmReportGeneration}>
          ठीक है
        </Button>
      </Modal.Footer>
    </Modal>
  ...
  return (
    <div ref={dashboardRef}>
                  ...
                  <Row>
                    ...
                    <Col md={3}>
                      <Button
                        ...
                        disabled={
                          !reportDateStart || !reportDateEnd || !reportType
                        }
                      >
                        रिपोर्ट जनरेट करें
                      </Button>
                ...
                <div className="filter-section mb-3 p-3 border rounded bg-light dashboard-graphs">
                  ...
                  <div className="date-range-filter mb-3 p-2 bg-white border rounded d-flex align-items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDateFilter((df) => ({ ...df, end: e.target.value }))
                      }
                    />
                    <Button
                      variant="primary"
                  ...
                  <Col lg={12} md={12} sm={12}>
                    <div className="dashboard-graphs p-3 border rounded bg-white">
                      ...
                      {(() => {
                        ...
                        const primaryColumn =
                          view === "detail" && selectedHierarchyLevel
                                ...
                                return (
                                  <div>
                                    <div className="d-flex justify-content-between align-items-center mb-3 dash-center-name">
                                      <h5 className="mb-0">
                                        {columnDefs[primaryColumn]?.label ||
                                          "Summary Table"}
                                      </h5>
                                      <div className="d-flex gap-1 align-items-center">
                                        ...
                                        >
                                          ...
                                          >
                                            <option value="sub_investment_name">
                                              उप-निवेश
                                            </option>
                                            <option value="investment_name">
                                              निवेश
                                            </option>
                                            <option value="scheme_name">
                                              योजना
                                            </option>
                                            <option value="center_name">
                                              केंद्र
                                            </option>
                                            <option value="vikas_khand_name">
                                              विकास खंड
                                            </option>
                                            <option value="vidhan_sabha_name">
                                              विधानसभा
                                            </option>
                                          </Form.Select>
                                        </div>
                                                                  ...
                                                                  <FormCheck
                                                                    ...
                                                                    onChange={() => {
                                                                      if (
                                                                        ...
                                                                        clearSummaryHeaderSelection(
                                                                          col,
                                                                        );
                                                                      } else {
                                                                        setSummaryHeaderSelectAll(
                                                                          col,
                                                                          allValues,
                                                                        );
                                                                      }
                                        ...
                                        <tbody>
                                          {(() => {
                                            ...
                                            try {
                                              ...
                                              console.error(
                                                "Error rendering hierarchical summary:",
                                                err,
                                              );
                                              return null;
                                            }
                                              ...
                                              .map((col) => {
                                                ...
                                                if (isExpanded) {
                                                  ...
                                                  if (
                                                    ...
                                                    const headerFilteredAll =
                                                      applySummaryHeaderFilters(
                                                        filteredData,
                                                      ).filter((row) =>
                                                        displayCheckedValues.includes(
                                                          row[
                                                            currentFilter.column
                                                          ],
                                                        ),
                                                                  ...
                                                                  (
                                                                    ...
                                                                    >
                                                                      ...
                                                                      >
                                                                        <span>
                                                                          {
                                                                            invItem.investmentName
                                                                          }
                                                                        </span>
                                                                      </div>
                                                                      {invItem.subInvestments.map(
                                                                        (
                                                                          subItem,
                                                                          subIdx,
                                                                        ) => (
                                                ...
                                                {(() => {
                                                  ...
                                                  const grandTotal =
                                                    headerFilteredAll
                                                      .reduce((sum, item) => {
                                                        const qtyVal =
                                                          typeof item.allocated_quantity ===
                                                            "string" &&
                                                          item.allocated_quantity.includes(
                                                            " / ",
                                                          )
                                                            ? parseFloat(
                                                                item.allocated_quantity.split(
                                                                  " / ",
                                                                )[0],
                                                              ) || 0
                                                            : parseFloat(
                                        ...
                                        <Row className="align-items-center">
                                          ...
                                          <Col md={5}>
                                            ...
                                            >
                                              ...
                                              यह फ़िल्टर नीचे दी गई सभी तालिकाओं
                                              पर लागू होता है
                                            </small>
                                          </Col>
                                        </Row>
                                    ...
                                    <div className="d-flex gap-2 mb-3">
                                      <Button
                                        ...
                                        size="sm"
                                        onClick={
                                          exportOpenCollapsibleTablesToPDF
                                        }
                                        disabled={
                                          !openCollapses.scheme &&
                                          !openCollapses.investment &&
                                          !openCollapses.subInvestment
                                        }
                                      >
                                      ...
                                      <Button
                                        ...
                                        size="sm"
                                        onClick={
                                          exportOpenCollapsibleTablesToExcel
                                        }
                                        disabled={
                                          !openCollapses.scheme &&
                                          !openCollapses.investment &&
                                      ...
                                      >
                                        <RiFileExcelLine className="me-1" />{" "}
                                        Excel निर्यात करें
                                      </Button>
                                    </div>
                                              ...
                                              {combinedTableData.investments
                                                ...
                                                <div className="table-responsive">
                                                  ...
                                                  >
                                                    ...
                                                    <tbody>
                                                      {combinedTableData.investments.map(
                                                        (investment, idx) => (
                                                          <tr key={investment}>
                                                            <td>{idx + 1}</td>
                                                            <td
                                                            ...
                                                            <td className="text-end font-weight-bold">
                                                              {formatCurrency(
                                                                combinedTableData.schemes.reduce(
                                                                  (sum, s) =>
                                                                    sum +
                                                                    ((combinedTableData
                                                                      .data[
                                                    ...
                                                    <tfoot>
                                                      ...
                                                      >
                                                        ...
                                                        {combinedTableData.schemes.map(
                                                          (scheme) => (
                                                            <>
                                                              ...
                                                              >
                                                                {formatCurrency(
                                                                  combinedTableData.investments.reduce(
                                                                    (
                                                                      sum,
                                                                      inv,
                                                                    ) =>
                                                        ...
                                                        <td className="text-end">
                                                          {(
                                                            combinedTableData.grandQuantity ||
                                                            0
                                                          ).toFixed(2)}
                                                        </td>
                                                        <td className="text-end">
                                    ...
                                    <Card className="chart-card mb-4">
                                      ...
                                      <Collapse
                                        in={openCollapses.subInvestment}
                                      >
                                        <div>
                                          <Card.Body>
                                            <div
                                            ...
                                            >
                                              {subCombinedTableData
                                                .subInvestments.length > 0 ? (
                                                <div className="table-responsive">
                                                  <table
                                                    id="subinvestment-table"
                                                    className="table table-sm table-striped mb-0"
                                                    style={{
                                                      fontSize: "0.75rem",
                                                      minWidth: "100%",
                                                  ...
                                                  >
                                                    ...
                                                    >
                                                      <tr>
                                                        ...
                                                        {subCombinedTableData.schemes.map(
                                                          (scheme) => (
                                                            <th
                                                              key={scheme}
                                                              className="text-center"
                                                    ...
                                                    <tbody>
                                                      {subCombinedTableData.subInvestments.map(
                                                        (
                                                          subInvestment,
                                                          idx,
                                                        ) => (
                                                          ...
                                                          >
                                                            ...
                                                            <td className="text-end">
                                                              {(
                                                                subCombinedTableData.quantities &&
                                                                (subCombinedTableData
                                                                  .quantities[
                                                                  subInvestment
                                                                ] ||
                                                            ...
                                                            <td className="text-end font-weight-bold">
                                                              {formatCurrency(
                                                                subCombinedTableData.schemes.reduce(
                                                                  (sum, s) =>
                                                                    sum +
                                                                    ((subCombinedTableData
                                                                      .data[
                                                    ...
                                                    <tfoot>
                                                      ...
                                                      >
                                                        ...
                                                        {subCombinedTableData.schemes.map(
                                                          (scheme) => (
                                                            <>
                                                              ...
                                                              >
                                                                {formatCurrency(
                                                                  subCombinedTableData.subInvestments.reduce(
                                                                    (
                                                                      sum,
                                                                      subInv,
                                                                    ) =>
                                                        ...
                                                        <td className="text-end">
                                                          {(
                                                            subCombinedTableData.grandQuantity ||
                                                            0
                                                          ).toFixed(2)}
                                                        </td>
                                                        <td className="text-end">
                                          ...
                                          <div className="mb-3">
                                            <Form.Group>
                                              ...
                                              <Select
                                                ...
                                                value={vidhanSabhaSchemeFilter.map(
                                                  (scheme) => ({
                                                    value: scheme,
                                                    label: scheme,
                                                  }),
                                                      ...
                                                      <FormCheck
                                                        ...
                                                        label={
                                                          ...
                                                          isRotated[index]
                                                            ? (() => {
                                                                ...
                                                                return additionalTableColumnFilters[
                                                                  index
                                                                ] &&
                                                        ...
                                                        onChange={() => {
                                                          if (
                                                            ...
                                                            handleAdditionalTableColumnFilterChange(
                                                              index,
                                                              "SELECT_ALL",
                                                            );
                                                          } else {
                                                  ...
                                                  <tr>
                                                    {(() => {
                                                      ...
                                                      if (isRot) {
                                                        ...
                                                        const visibleTransposed =
                                                          transposedTableColumns;
                                            ...
                                            <tbody>
                                              {(() => {
                                                ...
                                                if (isTableRotated) {
                                                  ...
                                                  const visibleColumnsBeforeRotation =
                                                    tableColumnFilters
                                                      .additional[index] ||
                                                    table.columns;
                                                  const visibleClickedCols =
                                                    allClickedCols.filter(
                                                      (col) =>
                                                        visibleColumnsBeforeRotation.includes(
                                                          col,
                                                        ),
                                                    );
                                                          ...
                                                          visibleTransposed.forEach(
                                                            (newCol) => {
                                                              if (
                                                                ...
                                                                return; // skip header label and total placeholder
                                                              const dataRow =
                                                                filteredData.find(
                                                                  (r) =>
                                                                    r[
                                                                ...
                                                                (
                                                                  ...
                                                                  if (
                                                                    ...
                                                                    if (
                                                                      ...
                                                                      const groupedData =
                                                                        {};
                                                                      currentFilteredData.forEach(
                                                                        (
                                                                          item,
                                                                        ) => {
                                                                          const investmentName =
                                                                            item.investment_name;
                                                                          const subInvestmentName =
                                                                            item.sub_investment_name;
                                                                      ...
                                                                      <td
                                                                        ...
                                                                        style={{
                                                                          maxWidth:
                                                                            "200px",
                                                                          height:
                                                                            "100%", // Ensure cell takes full height
                                                                        }}
                                                                      >
                                                                        <div
                                                                          style={{
                                                                            ...
                                                                            flexDirection:
                                                                              "column",
                                                                            justifyContent:
                                                                              "space-between", // Evenly space centers
                                                                            maxHeight:
                                                                              "150px",
                                                                            overflowY:
                                                                              "auto",
                                                                                ...
                                                                                style={{
                                                                                  ...
                                                                                  borderBottom:
                                                                                    ...
                                                                                    uniqueValues.length -
                                                                                      1
                                                                                      ? "1px solid #eee"
                                                                                      : "none", // Separator between centers
                                                  ...
                                                  <tfoot>
                                                    ...
                                                    >
                                                      {(() => {
                                                        ...
                                                        const detectedUnitRow =
                                                          filteredData.find(
                                                            (r) =>
                                                              r.unit ||
                                                              r.investment_unit ||
                                                              r[
                                                                `${table.columns[0]}_unit`
                                                              ],
                                                          );
                                                        const detectedUnit =
                                                          detectedUnitRow
                                                            ? detectedUnitRow.unit ||
                                                              detectedUnitRow.investment_unit ||
                                                              getUnitFromItem(
                                                                detectedUnitRow,
                                                              )
                                                            : "";
                      ...
                      <div className="graph-visualization-section mt-4 p-3 border rounded bg-light">
                        <div className="d-flex justify-content-between align-items-center mb-3">
                          ...
                          {Object.values(graphFilters).some(
      ...
      >
        <Modal.Header closeButton className="modal-header-style">
          <div>
            {detailPreviewType === "pdf"
              ? "Detail Table - PDF पूर्वालोकन"
              : "Detail Table - Excel पूर्वालोकन"}
          </div>
        </Modal.Header>
        <Modal.Body style={{ maxHeight: "70vh", overflow: "auto" }}>
        ...
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowDetailPreviewModal(false)}
          >
            बंद करें
          </Button>
          <Button
        ...
        <Modal.Header closeButton className="modal-header-style">
          <div>
            {allocationPreviewType === "pdf"
              ? "Allocation Table - PDF पूर्वालोकन"
              : "Allocation Table - Excel पूर्वालोकन"}
          </div>
        </Modal.Header>
        <Modal.Body style={{ maxHeight: "70vh", overflow: "auto" }}>
          {allocationPreviewType === "pdf" ? (
            ...
            <div
              dangerouslySetInnerHTML={{ __html: allocationPreviewContent }}
            />
          )}
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowAllocationPreviewModal(false)}
          >
            बंद करें
          </Button>
          <Button
      ...
      >
        <Modal.Body style={{ textAlign: "center", padding: "40px 20px" }}>
          ...
          <h5 style={{ marginTop: "20px", color: "#333" }}>
