...
const readJsonResponse = async (response) => {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    ...
    if (response.status === 401) {
      throw new Error("सत्र समाप्त हो गया है। कृपया दोबारा लॉगिन करें।");
    }

    if (response.status === 403) {
      throw new Error("आपको इस कार्य की अनुमति नहीं है।");
    }

    if (response.status === 404) {
      throw new Error("API URL उपलब्ध नहीं है। कृपया backend API URL जाँचें।");
    }
...
const belowThousand = (number) => {
  ...
  if (hundred) {
    result += HINDI_NUMBERS[hundred] + " सौ ";
  }
...
const numberToHindiWords = (value) => {
  ...
  if (crore) {
    result += belowThousand(crore) + " करोड़ ";
  }

  if (lakh) {
    result += belowThousand(lakh) + " लाख ";
  }

  if (thousand) {
    result += belowThousand(thousand) + " हजार ";
  }
...
const amountToHindiWords = (value) => {
  ...
  if (paise > 0) {
    text += " " + numberToHindiWords(paise) + " पैसे";
  }
...
const buildBillPayload = ({
  ...
  financial_year: String(financialYear ?? bill.financial_year ?? "").trim(),
...
export default function CenterUdyanBill() {
  ...
  const [standardForm, setStandardForm] = useState(emptyStandard);
  ...
  useEffect(() => {
    if (centerData?.centerName && !bill.center) {
      setBill((previous) => ({
        ...previous,
        center: centerData.centerName,
      }));
  ...
  const loadStandards = async () => {
    ...
    try {
      ...
      if (
        selectedCropId &&
        !activeList.some((item) => String(item.id) === String(selectedCropId))
      ) {
        setSelectedCropId("");
  ...
  const loadUploadedBills = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || data?.error || "बिल सूची प्राप्त नहीं हो सकी।",
        );
      }
  ...
  const deleteBill = async (billId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error || `बिल #${billId} हटाया नहीं जा सका।`);
      }

      // If the deleted bill was currently open in the form,
      // leave edit mode and clear the form.
      if (editingBillId !== null && String(editingBillId) === String(billId)) {
        setBill({
          ...emptyBill,
      ...
      setMessage(`बिल #${billId} सफलतापूर्वक हटा दिया गया।`);
    } catch (error) {
      console.error("deleteBill:", error);

      alert(error.message || `बिल #${billId} हटाया नहीं जा सका।`);
    } finally {
  ...
  const selectedStandard = useMemo(() => {
    return (
      standards.find((item) => String(item.id) === String(selectedCropId)) ||
      null
    );
  ...
  const calculation = useMemo(() => {
    ...
    if (bill.calculation_basis === "plant") {
      ...
      manureTotal =
        (standardTotal -
          plantRate * plantsPerHectare -
          pitRate * plantsPerHectare) *
        area;
    }
    ...
    const manureSubsidy = Math.min(
      manureTotal,
      Math.max(0, standardSubsidy * area - plantSubsidy),
    );
  ...
  const openEditStandard = (standard) => {
    ...
    setStandardForm({
      financial_year: standard.financial_year || financialYear,

      crop_name: standard.crop_name || "",

      spacing: standard.spacing || "",

      plants_per_hectare: standard.plants_per_hectare ?? "",

      plant_rate: standard.plant_rate ?? "",

      pit_rate: standard.pit_rate ?? "",

      manure_rate: standard.manure_rate ?? "",

      manure_quantity: standard.manure_quantity ?? "",

      standard_total: standard.standard_total ?? "",

      standard_subsidy: standard.standard_subsidy ?? "",
  ...
  const saveStandard = async () => {
    if (!String(standardForm.financial_year).trim()) {
      alert("वित्तीय वर्ष दर्ज करें।");
      return;
    }

    if (!String(standardForm.crop_name).trim()) {
      alert("फसल का नाम दर्ज करें।");
      return;
    }

    if (!String(standardForm.spacing).trim()) {
      alert("दूरी दर्ज करें।");
      return;
    }

    const total = num(standardForm.standard_total);

    const subsidy = num(standardForm.standard_subsidy);

    if (subsidy > total) {
      alert("राजसहायता मानक महायोग से अधिक नहीं हो सकती।");
      return;
    }
    ...
    const payload = {
      financial_year: String(standardForm.financial_year).trim(),

      crop_name: String(standardForm.crop_name).trim(),

      spacing: String(standardForm.spacing).trim(),

      plants_per_hectare: num(standardForm.plants_per_hectare),

      plant_rate: num(standardForm.plant_rate),

      pit_rate: num(standardForm.pit_rate),

      manure_rate: num(standardForm.manure_rate),

      manure_quantity: num(standardForm.manure_quantity),

      standard_total: total,

      standard_subsidy: subsidy,

      is_active: Boolean(standardForm.is_active),
    };
    ...
    try {
      ...
      console.error("saveStandard:", error);
  ...
  const deleteStandard = async (id) => {
    if (!window.confirm("क्या आप इस मानक को हटाना चाहते हैं?")) {
      return;
    }

    try {
      const response = await apiFetch(`${API_BASE}/crop-standards/${id}/`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(error || "मानक हटाया नहीं जा सका।");
      }

      if (String(selectedCropId) === String(id)) {
        setSelectedCropId("");
      ...
      setMessage("मानक सफलतापूर्वक हटाया गया।");
    } catch (error) {
      console.error("deleteStandard:", error);

      alert(error.message || "मानक हटाया नहीं जा सका।");
    }
  };
  ...
  const saveBill = async () => {
    if (!selectedStandard) {
      alert("कृपया फल पौध का मानक चुनें।");
      return;
    }

    if (!String(bill.farmer_name ?? "").trim()) {
      alert("कृपया कृषक का नाम दर्ज करें।");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error ||
            (isEditing ? "बिल अपडेट नहीं हो सका।" : "बिल सेव नहीं हो सका।"),
        );
      }
  ...
  const resetBill = () => {
    if (!window.confirm("भरी गई सभी सूचनाएँ मिटा दी जाएँगी। जारी रखें?")) {
      return;
    }
    ...
    <div className="uploaded-bills-panel">
      ...
      {loadingBills ? (
        ...
        <div className="no-standards">
          <h3>अभी कोई बिल उपलब्ध नहीं है</h3>
          <p>सेव किया गया बिल यहाँ दिखाई देगा।</p>
        </div>
      ) : (
            ...
            <tbody>
              {uploadedBills.map((savedBill, index) => (
                <tr key={savedBill.id ?? index}>
                  ...
                  <td>
                    <strong>{savedBill.farmer_name || "—"}</strong>
                    <small>{savedBill.village || ""}</small>
                  </td>
                  <td>{savedBill.crop_name || savedBill.crop || "—"}</td>
                  <td>{savedBill.center || "—"}</td>
                  <td>{savedBill.area || "0.00"}</td>
                  <td className="money">
                    ₹ {formatMoney(savedBill.grand_total)}
                  </td>
                  <td>
                    {savedBill.created_at
                      ? new Date(savedBill.created_at).toLocaleDateString(
                          "hi-IN",
                        )
                      : "—"}
                  </td>
    ...
    if (!selectedStandard) {
      return (
        <div className="preview-empty">
          ...
          <p>
            ऊपर से फल पौध का मानक चुनें। चयन करते ही यहाँ A4 प्रपत्र स्वतः तैयार
            होगा।
          </p>
        </div>
      );
      ...
      <div className="document-area" id="printDocument">
        ...
        <div className="a4-page">
          ...
          <div className="document-center document-title">
            जिला योजनान्तर्गत उद्यान स्थापना — वर्ष {financialYear} &nbsp;(बिल)
          </div>
          ...
          <table className="document-table bill-table">
            <thead>
              <tr>
                ...
                <th colSpan="3">
                  व्यय का विवरण
                  <br />
                  (क्षेत्रफल {num(bill.area).toFixed(2)} है0 हेतु)
                </th>
              </tr>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  गोबर खाद/जैविक एवं वर्मी कम्पोस्ट/अन्य पोषक तत्व/पौध
                  सुरक्षा/रोपण सिंचाई आदि (कु0) @ रु0{" "}
                  {selectedStandard.manure_rate} प्रति कु0
                </td>

                <td className="center">{calculation.manureQuantity} कु0</td>

                <td className="money">
                  ₹{formatMoney(calculation.manureTotal, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.manureSubsidy, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.manureFarmer, digits)}
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  गड्ढा खुदान, भरान, पौध रोपण (1×1×1 मी0) @ रु0{" "}
                  {selectedStandard.pit_rate} प्रति गड्ढा
                </td>

                <td className="center">{calculation.plants} गड्ढा</td>

                <td className="money">
                  ₹{formatMoney(calculation.pitTotal, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.pitSubsidy, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.pitFarmer, digits)}
                </td>
              </tr>
              ...
              <tr className="total-row">
                ...
                <td className="money">
                  ₹{formatMoney(calculation.billTotal, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.billSubsidy, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.billFarmer, digits)}
                </td>
              </tr>
              ...
              <tr>
                ...
                <td className="small-text">
                  फल पौध की लागत ({selectedStandard.spacing}) @ रु0{" "}
                  {selectedStandard.plant_rate} प्रति पौध — विभाग द्वारा पौध के
                  रूप में आपूर्ति (नगद देय नहीं)
                </td>

                <td className="center">{calculation.plants}</td>

                <td className="money">
                  ₹{formatMoney(calculation.plantTotal, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.plantSubsidy, digits)}
                </td>
              ...
              <tr className="grand-row">
                ...
                <td className="money">
                  ₹{formatMoney(calculation.grandTotal, digits)}
                </td>

                <td className="money">
                  ₹{formatMoney(calculation.grandSubsidy, digits)}
                </td>
          ...
          <p className="document-paragraph">
            ...
            <strong>{num(bill.area).toFixed(2)}</strong> है0 क्षेत्रफल में{" "}
            <strong>{selectedStandard.crop_name}</strong> उद्यान लगाने हेतु मु0
            रु0 <strong>{formatMoney(calculation.billTotal, digits)}</strong> (
            {amountToHindiWords(calculation.billTotal)}) की धनराशि का कुल व्यय
            किया गया है, अतः कार्य योजना के अनुसार क्र0 सं0 1 व 2 की धनराशि मु0
            रु0 <strong>{formatMoney(calculation.billSubsidy, digits)}</strong>{" "}
            ({amountToHindiWords(calculation.billSubsidy)}) राजसहायता का भुगतान
          ...
          <p className="document-paragraph">
            प्रमाणित किया जाता है कि कृषक द्वारा उक्तानुसार{" "}
            <strong>{num(bill.area).toFixed(2)}</strong> है0 उद्यान लगाने हेतु{" "}
            <strong>{calculation.plants}</strong> पौधों का रोपण किया गया है,
            जिसका मेरे द्वारा स्थलीय सत्यापन कर दिया गया है। अतः कृषक को योजना
            अनुसार मु0 रु0{" "}
            <strong>{formatMoney(calculation.billSubsidy, digits)}</strong> (
            {amountToHindiWords(calculation.billSubsidy)}) का अनुदान भुगतान करने
            की संस्तुति की जाती है।
          </p>
        ...
        <div className="a4-page">
          ...
          <div className="document-center document-title-small">
            गोबर खाद/जैविक एवं वर्मी कम्पोस्ट/ अन्य पोषक तत्व आदि
          </div>

          <div className="document-center document-line">
            फल पौध : <strong>{selectedStandard.crop_name}</strong>
            &nbsp;&nbsp; क्षेत्रफल :{" "}
            <strong>{num(bill.area).toFixed(2)}</strong> है0 &nbsp;&nbsp; वर्ष :{" "}
            <strong>{financialYear}</strong>
          </div>

          <p className="document-paragraph large-gap">
            मु0 रु0{" "}
            <strong>{formatMoney(calculation.manureTotal, digits)}</strong> (
            {amountToHindiWords(calculation.manureTotal)}) बावत गोबर खाद/जैविक
            एवं वर्मी कम्पोस्ट/अन्य पोषक तत्व आदि का भुगतान रु0{" "}
            <strong>{formatMoney(calculation.manureTotal, digits)}</strong> श्री{" "}
            <DocField
              bill={bill}
              onChange={updateBill}
              field="farmer_name"
          ...
          <p className="document-paragraph large-gap">
            प्रमाणित किया जाता है कि मेरे द्वारा श्री{" "}
            <DocField
              bill={bill}
              onChange={updateBill}
              field="supplier_name"
            ...
            <DocField
              ...
              className="inline-document-input"
            />{" "}
            को <strong>{num(bill.area).toFixed(2)}</strong> है0 में गोबर
            खाद/जैविक एवं वर्मी कम्पोस्ट/अन्य पोषक तत्व आदि हेतु मु0 रु0{" "}
            <strong>{formatMoney(calculation.manureTotal, digits)}</strong> (
            {amountToHindiWords(calculation.manureTotal)}) का नगद भुगतान किया
            गया है। अतः राजसहायता का भुगतान रु0{" "}
            <strong>{formatMoney(calculation.manureSubsidy, digits)}</strong> (
            {amountToHindiWords(calculation.manureSubsidy)}) मुझे करने की कृपा
            कीजियेगा।
          </p>
          ...
          <p className="document-paragraph large-gap">
            ...
            <strong>{bill.farmer_name || "________"}</strong> पुत्र श्री{" "}
            <strong>{bill.father_husband_name || "________"}</strong> ग्राम{" "}
            <strong>{bill.village || "________"}</strong> द्वारा{" "}
            <strong>{calculation.manureQuantity}</strong> गोबर खाद/जैविक एवं
            वर्मी कम्पोस्ट/अन्य पोषक तत्व आदि का कार्य किया गया है, जिसका स्थलीय
            सत्यापन मेरे द्वारा कर दिया गया है। अतः राजसहायता रु0{" "}
            <strong>{formatMoney(calculation.manureSubsidy, digits)}</strong> (
            {amountToHindiWords(calculation.manureSubsidy)}) का भुगतान करने की
            संस्तुति की जाती है।
          </p>
        ...
        {showVoucher2 && (
          <div className="a4-page">
            ...
            <div className="document-center document-title-small">
              गड्ढा खुदान, भरान, पौध रोपण (1×1×1 मी0)
            </div>
            ...
            <p className="document-paragraph large-gap">
              मु0 रु0{" "}
              <strong>{formatMoney(calculation.pitTotal, digits)}</strong> (
              {amountToHindiWords(calculation.pitTotal)}) बावत गड्ढा खुदान,
              भरान, पौध रोपण (1×1×1 मी0) का भुगतान रु0{" "}
              <strong>{formatMoney(calculation.pitTotal, digits)}</strong> श्री{" "}
              <DocField
                bill={bill}
                onChange={updateBill}
                field="farmer_name"
            ...
            <p className="document-paragraph large-gap">
              प्रमाणित किया जाता है कि मेरे द्वारा श्री{" "}
              <strong>{bill.labour_name || "________"}</strong> पुत्र श्री{" "}
              <strong>{bill.labour_father_name || "________"}</strong> ग्राम{" "}
              <strong>{bill.labour_village || "________"}</strong> को{" "}
              <strong>{num(bill.area).toFixed(2)}</strong> है0 में गड्ढा खुदान,
              भरान, पौध रोपण (1×1×1 मी0) हेतु मु0 रु0{" "}
              <strong>{formatMoney(calculation.pitTotal, digits)}</strong> (
              {amountToHindiWords(calculation.pitTotal)}) का नगद भुगतान किया गया
              है।
            </p>
            ...
            <table className="standard-calc-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        update("plants_per_hectare", e.target.value)
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td className="money">
                    {formatMoney(calculated.plantTotal, 2)}
                  </td>
                  <td className="money">
                    {formatMoney(calculated.plantSubsidy, 2)}
                  </td>
                ...
                <tr>
                  ...
                  <td className="money">
                    {formatMoney(calculated.pitTotal, 2)}
                  </td>
                  <td className="money">0.00</td>
                  <td className="money">
                    {formatMoney(calculated.pitFarmer, 2)}
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td>
                    गोबर खाद/जैविक एवं वर्मी कम्पोस्ट/पोषक तत्व/पौध सुरक्षा/रोपण
                    सिंचाई
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        update("manure_quantity", e.target.value)
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td className="money">
                    {formatMoney(calculated.manureTotal, 2)}
                  </td>
                  <td className="money">
                    {formatMoney(calculated.manureSubsidy, 2)}
                  </td>
                  <td className="money">
                    {formatMoney(calculated.manureFarmer, 2)}
                  </td>
                </tr>
                ...
                <tr className="standard-sum-row">
                  ...
                  <td className="money">
                    {formatMoney(calculated.subsidy, 2)}
                  </td>
        ...
        <div className="standards-manager">
          <div className="standards-toolbar">
            <div>
              <h2>मानक तालिका</h2>
              <p>
                फसलवार मानक डेटाबेस से आते हैं। पीले खानों में मान बदलने पर नीचे
                की गणना तुरन्त बदलती है।
              </p>
            </div>
            <button
              type="button"
      ...
      <div className="control-panel">
        <div className="control-heading">
          <div>
            ...
            <p>स्वतः गणना प्रपत्र — उपयोगकर्ता द्वारा दर्ज फसल मानक</p>
          </div>
        ...
        <div className="control-tabs">
          <button
            type="button"
            className={activeSection === "bill" ? "active" : ""}
          ...
          <button
            type="button"
            className={activeSection === "standards" ? "active" : ""}
          ...
          <>
            ...
            <div className="quick-controls">
              ...
              <label className="control-field">
                <span>क्षेत्रफल है0 (सीधे अंकों में लिखें)</span>

                <input
                  value={bill.area}
                  inputMode="decimal"
                  onChange={(event) => updateBill("area", event.target.value)}
                  placeholder="जैसे 0.125"
                />
              </label>

              <label className="control-field">
                <span>पौध संख्या (स्वतः / बदल सकते हैं)</span>

                <input
                  type="number"
                  value={
                    bill.calculation_basis === "area"
                      ? calculation.plants
                      : bill.plants
                  }
                  disabled={bill.calculation_basis === "area"}
                  onChange={(event) => updateBill("plants", event.target.value)}
                />
              </label>
              ...
              <label className="control-field">
                ...
                <select
                  value={bill.calculation_basis}
                  onChange={(event) =>
                    updateBill("calculation_basis", event.target.value)
                  }
                >
              ...
              <label className="control-field">
                ...
                <select
                  value={bill.rounding}
                  onChange={(event) =>
                    updateBill("rounding", event.target.value)
                  }
                >
              ...
              <label className="control-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setShowVoucher2(event.target.value === "yes")
                  }
                >
            ...
            <div className="action-row">
              ...
              <label className="print-check">
                <input
                  type="checkbox"
                  checked={includeStandardPrint}
                  onChange={(event) =>
                    setIncludeStandardPrint(event.target.checked)
                  }
                />
            ...
            {selectedStandard && (
              <div className="live-tally">
                <div>
                  ...
                  <strong>
                    ₹
                    {formatMoney(calculation.grandTotal, Number(bill.rounding))}
                  </strong>
                </div>

                <div>
                  <span>कुल राजसहायता</span>
                ...
                <div>
                  ...
                  <strong>
                    ₹
                    {formatMoney(calculation.billFarmer, Number(bill.rounding))}
                  </strong>
                </div>
            ...
            <details className="input-section" open>
              <summary>प्रपत्र में भरने योग्य विवरण</summary>
            ...
            <div className="modal-body">
              <div className="modal-grid">
                <label>
                  ...
                  <input
                    value={standardForm.financial_year}
                    onChange={(event) =>
                      updateStandardForm("financial_year", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>फल पौध / फसल का नाम *</span>

                  <input
                    value={standardForm.crop_name}
                    placeholder="जैसे आम कलमी"
                    onChange={(event) =>
                      updateStandardForm("crop_name", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>दूरी / Spacing *</span>

                  <input
                    value={standardForm.spacing}
                    placeholder="जैसे 8 × 8 मी0"
                    onChange={(event) =>
                      updateStandardForm("spacing", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>पौध संख्या प्रति है0 *</span>

                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={standardForm.plants_per_hectare}
                    onChange={(event) =>
                      updateStandardForm(
                        "plants_per_hectare",
                        event.target.value,
                ...
                <label>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={standardForm.plant_rate}
                    onChange={(event) =>
                      updateStandardForm("plant_rate", event.target.value)
                    }
                  />
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={standardForm.pit_rate}
                    onChange={(event) =>
                      updateStandardForm("pit_rate", event.target.value)
                    }
                  />
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={standardForm.manure_rate}
                    onChange={(event) =>
                      updateStandardForm("manure_rate", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>खाद मात्रा</span>

                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={standardForm.manure_quantity}
                    onChange={(event) =>
                      updateStandardForm("manure_quantity", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>मानक महायोग *</span>

                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={standardForm.standard_total}
                    onChange={(event) =>
                      updateStandardForm("standard_total", event.target.value)
                    }
                  />
                </label>

                <label>
                  <span>देय राजसहायता *</span>

                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={standardForm.standard_subsidy}
                    onChange={(event) =>
                      updateStandardForm("standard_subsidy", event.target.value)
                    }
                  />
                </label>
                ...
                <label className="active-checkbox">
                  <input
                    type="checkbox"
                    checked={standardForm.is_active}
                    onChange={(event) =>
                      updateStandardForm("is_active", event.target.checked)
                    }
                  />

                  <span>यह मानक सक्रिय है</span>
                </label>
              </div>
              ...
              <div className="modal-note">
                ...
                <span>यहाँ दर्ज किया गया मानक database में सेव होगा।</span>
              </div>
            </div>
